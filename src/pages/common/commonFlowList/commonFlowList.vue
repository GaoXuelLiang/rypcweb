        ...
        getColumns () {
            ...
            getDataLevelUserLoginData({
                ...
                if (isSuccess(res, t)) {
                    ...
                    // 通过返回列表字段的
                    // columns = columns.sort(this.compare('age'))
                    aa = columns
                    for (let i = 0; i < aa.length; i++) {
                                        ...
                                        h('div', {
                                            ...
                                            on: {
                                                click: async () => {
                                                    ...
                                                    if (this.stepState === 'p_flowst_0' || stepAuth === 'none') {
                                                        return
                                                    }
                        ...
                        if (!aa[i].width) {
                            aa[i].width = t.sumWidth(aa[i].title, aa[i].className);
                        }
                    }
                    let dom = document.getElementsByClassName('ivu-card-body')[0].offsetWidth
                    if (this.$global.mt == 'xian') {
                        for (let j in aa) {
                            if (dom > 1500 && aa.length > 13) {
                                aa[0].fixed = 'left'
                                aa[1].fixed = 'left'
                                aa[2].fixed = 'left'
                            }
...
.table /deep/ .ivu-table-fixed-body{
    z-index: 0 !important;
}
</style>
