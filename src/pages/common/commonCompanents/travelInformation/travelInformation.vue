<template>
  <div class="table">
      ...
      <i-Col span="24">
        <card>
          ...
          <row class="table-form" ref="table-form">
            ...
            <Table @on-select="selectedtable" @on-select-cancel="selectedtable"  @on-select-all="selectedtable" @on-select-all-cancel="selectedtable" @on-sort-change="sortable" :height="tableheight" size="small"
    ...
    <transition name="fade">
      <update v-show="openEduUpdate" :flowInfo="flowInfo" :id="updateId" :logType="logType" :index="index" @closeUp="closeUps" @getData="addNewArray" @update="updateArray" ref="update"></update>
    </transition>
  </div>
</template>
<script>
...
//      t.getSelect()
    ...
    methods: {
      ...
      getData(page, value) {
        const t = this
//        console.log(123, t.flowInfo)
        if (page) {
          t.page = page
        }
        const data = {
          _mt:  this.$global.mt+'ApplyTravelrecord.getPage',
          rows: t.rows,
          page: t.page,
          sort: t.sort,
          order: t.order,
          logType: '流程子集',
          apptrrecTrid: t.flowInfo.flowId,
          apptrrecDataid: t.flowInfo.pkValue
        }
...
//          console.log(t.tableselected.toString())
          t.$Modal.confirm({
            title: this.$t('reminder.remind'),
            content: this.$t('reminder.confirmdelete'),
            onOk: () => {
              getDataLevelUserLogin({
                _mt:  this.$global.mt+'ProReport.delByIds',
                funId: '1',
