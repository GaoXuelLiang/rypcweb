          ...
          <row class="table-form" ref="table-form">
            ...
            <Table :columns="columns" :data="data">
              ...
              <template slot-scope="{ row, index }" slot="proevaPerf">
                <Input type="text" v-model="proevaPerf" v-if="editIndex === index"  placeholder="请填写"/>
                <span v-else>{{ row.proevaPerf }}</span>
              </template>

              <template slot-scope="{ row, index }" slot="proevaExp">
                <Input type="text" v-model="proevaExp" v-if="editIndex === index"  placeholder="请填写"/>
                <span v-else>{{ row.proevaExp }}</span>
              </template>

              <template slot-scope="{ row, index }" slot="proevaCreate">
                <Input type="text" v-model="proevaCreate" v-if="editIndex === index"  placeholder="请填写"/>
                <span v-else>{{ row.proevaCreate }}</span>
              </template>

              <template slot-scope="{ row, index }" slot="proevaOrg">
                <Input type="text" v-model="proevaOrg" v-if="editIndex === index"  placeholder="请填写"/>
                <span v-else>{{ row.proevaOrg }}</span>
              </template>

              <template slot-scope="{ row, index }" slot="proevaAttitude">
                <Input type="text" v-model="proevaAttitude" v-if="editIndex === index"  placeholder="请填写"/>
                <span v-else>{{ row.proevaAttitude }}</span>
              </template>

              <template slot-scope="{ row, index }" slot="proevaEfficiency">
                <Input type="text" v-model="proevaEfficiency" v-if="editIndex === index"  placeholder="请填写"/>
                <span v-else>{{ row.proevaEfficiency }}</span>
              </template>

              <template slot-scope="{ row, index }" slot="proevaComm">
                <Input type="text" v-model="proevaComm" v-if="editIndex === index"  placeholder="请填写"/>
                <span v-else>{{ row.proevaComm }}</span>
              </template>

              <template slot-scope="{ row, index }" slot="proevaFit">
                <Input type="text" v-model="proevaFit" v-if="editIndex === index"  placeholder="请填写"/>
                <span v-else>{{ row.proevaFit }}</span>
              </template>

              <template slot-scope="{ row, index }" slot="proevaStudy">
                <Input type="text" v-model="proevaStudy" v-if="editIndex === index"  placeholder="请填写"/>
                <span v-else>{{ row.proevaStudy }}</span>
              </template>

              <template slot-scope="{ row, index }" slot="proevaPlasticity">
                <Input type="text" v-model="proevaPlasticity" v-if="editIndex === index"  placeholder="请填写"/>
              ...
              <template slot-scope="{ row, index }" slot="action" v-if="btnRole">
                <div v-if="editIndex === index">
                  <Button @click="handleSave(index)">保存</Button>
                  <Button @click="editIndex = -1">取消</Button>
                </div>
                <div v-else>
                  <Button @click="handleEdit(row, index)" :disabled="row.proevaTester === '自评分数'">修改</Button>
                </div>
              </template>
            </Table>
          </row>
...
//          console.log(res)
          ...
          if (isSuccess(res, t)) {
            if (res.data.content[0].rows.length > 0) {
              if (res.data.content[0].rows.length === 1) {
                t.data[1].proevaTester = res.data.content[0].rows[0].proevaTester
                  t.data[1].proevaPerf = res.data.content[0].rows[0].proevaPerf
                  t.data[1].proevaExp = res.data.content[0].rows[0].proevaExp
                  t.data[1].proevaCreate = res.data.content[0].rows[0].proevaCreate
                  t.data[1].proevaOrg = res.data.content[0].rows[0].proevaOrg
                  t.data[1].proevaAttitude = res.data.content[0].rows[0].proevaAttitude
                  t.data[1].proevaEfficiency = res.data.content[0].rows[0].proevaEfficiency
                  t.data[1].proevaComm = res.data.content[0].rows[0].proevaComm
                  t.data[1].proevaFit = res.data.content[0].rows[0].proevaFit
                  t.data[1].proevaStudy = res.data.content[0].rows[0].proevaStudy
                  t.data[1].proevaPlasticity = res.data.content[0].rows[0].proevaPlasticity
                  t.data[1].proevaTotal = res.data.content[0].rows[0].proevaTotal
              }
              if (res.data.content[0].rows.length === 2) {
...
//          console.log(id)
      ...
      handleEdit(row, index) {
        ...
        this.proevaTester = row.proevaTester
