  ...
  <div class="background">
    <div class="backbox">
      ...
      <Row class="table-form">
        <i-col span="14">
          <span @dblclick="clearData()" v-if="title === '选择基准岗位'">
          ...
          <span style="margin: 0;"><Button type="primary"  @click="getData(param, 1)">{{$t('button.ser')}}</Button></span>
        </i-col>
      ...
      <Row>
        <i-col span="14">
          <row class="table-form" ref="table-form">
            <Table height="320" size="small" border ref="selection" :columns="searchCloumns" @on-sort-change="sortable" :data="data" @on-row-dblclick="dbCkick" @on-select="selectedtable" @on-select-cancel="selectedtableCancel"  @on-select-all="selectedtable" @on-select-all-cancel="selectedtableCancel"></Table>
          </row>
        </i-col>
      ...
      <Row v-if="autoTableType !== 'unit'" style="text-align: right">
        <Button type="primary" @click="handleSubmit" v-if="orgPostTag.length > 0">保存</Button>
      </Row>
    </div>
    <autoSearchtable
      v-if="showModal"
