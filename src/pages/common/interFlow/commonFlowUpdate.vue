        ...
        <div class="content">
          ...
          <div class="dataBlocks" v-for="(item, index) in dataBlocks" :key="index">
            ...
            <div class="dataContent">
              ...
              <row class="table-form" ref="table-form" v-for="(items,indexs) in columnsChildAll">
                <row v-for="itemTable in dataTable" v-if="!item.blockColumn && item.id == itemTable.dataBlockId">
                  <Table @on-selection-change="selectedtable" v-if="!item.blockColumn && item.id == items[0].dataBlockId"  border ref="selection" size="small" :columns="items" :data="itemTable.table"></Table>
                </row>
              </row>
              <row v-if="!item.blockColumn">
                <Row style="display: flex">          <Page :total="totalTable" size="small" show-elevator show-sizer placement="top" @on-page-size-change="sizeChange" @on-change="pageChange" :page-size-opts = "[10, 20, 50, 100]" ></Page>
...
//        console.log(params)
      ...
      ChildopenUp(params, paramsRow, paramsAll) {
        console.log(params, paramsRow, paramsAll)
        console.log(this.dataTable)``
        console.log(this.ChildDataBloks)
        for (let i = 0; i < this.dataTable.length; i++) {
            for (let j = 0; j < JSON.parse(this.dataTable[i].rows).length; j++) {
              console.log(JSON.parse(this.dataTable[i].rows)[j])
              if (JSON.parse(this.dataTable[i].rows)[j].empnhfmCname === paramsAll.row.empnhfmCname) {
...
//        console.info(t.docs)
      ...
      btnFunction(flsdbSubformid, flsdbSubformtype, flsdbSubfilter) {
        ...
        this.openTestUpd = true
        this.Tabledisabled = false
        console.log(this.flsdbSubform, this.flsdbSubformtype, this.flsdbSubfilter)
      },
      closeTest() {
...
//                aa[i].width = 120
                aa[i]['render'] = (h, params) => {
                  ...
                  this.thisSetpName = this.setpName
                  let bb = []
...
//            console.log(t.columns, '396')
      ...
      getData() {
        ...
        getDataLevelUserLoginId({
          ...
          if (isSuccess(res, t)) {
//            console.log(res.data.content[0].rows + '1')
...
//          console.log(res.data.content[0].rows + '2')
          ...
          console.log(t.dataTable, '1111111111111111111111111111111111111111111')
//          console.log(t.dataTableAll, '22222222222222222222222222222222222')
...
//          console.log(t.columnsChild, '33333333333333333333333')
//          console.log(t.columnsChildAll, '444444444444444444444444')
        }).catch(err => {
          console.log(err)
        })
      },
