  ...
  <div class="table">
    <Row>
      ...
      <card>
        ...
        <row class="table-form table-form1" ref="table-form">
...
<script>
  ...
  export default{
    data() {
      return {
        tableheight: document.body.offsetHeight - 160,
        value: '',
        logType: '',
        openUpdate: false,
        openTestUpd: false,
...
//                          height: '100%',
      ...
      getData() {
        ...
        if ('530200' === t.$store.state.user.funCode) {
          const data = {"queryCriteria":[{"empdimApplicant": t.$store.state.user.userId},{"teSing": "teSing"},{"sysUsempid": t.$store.state.user.sysUsempid}]}
          getCri = JSON.stringify(data)
        } else if ('510600' === t.$store.state.user.funCode) {
//          console.log("99882522")
          const data =  {"queryCriteria":[{"sysUsempid": t.$store.state.user.sysUsempid}]}
          getCri = JSON.stringify(data)
