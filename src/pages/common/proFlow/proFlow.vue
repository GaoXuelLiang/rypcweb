        ...
        <Row>
            ...
            <card>
                <p slot="title">
                    ...
                    &nbsp;{{$t('lang_communication.proFlow.title')}}
                </p>
                <Row>
                    <i-col span="24">
                        <!--<span @dblclick="clearKnowType">-->
                        <!--问题类型：<Input v-model="this.$store.state.myWorkOrderNew.ProblemTypeName"  icon="ios-search" :readonly="true" placeholder="请选择业务工单类型" style="width: 160px;margin-right: 10px;" @on-click="pickData" />-->
                        <!--</span>-->
                        {{$t('lang_communication.workorderSearch.cmwdCode')}}：<Input :placeholder="$t('lang_communication.workorderSearch.cmwdCodeDis')"
                               v-model="cmwdCode"
                               style="width: 100px;" />
                        ...
                        {{$t('lang_communication.myWorkOrderNew.cmwdkeyWord')}}：<Input :placeholder="$t('lang_communication.myWorkOrderNew.cmwdkeyWord')"
                               v-model="cmwdkeyWord"
                               style="width: 100px;" />
                        {{$t('lang_communication.proFlow.flowWorkno')}}：<Input :placeholder="$t('lang_communication.proFlow.flowWorknoDis')"
                               v-model="flowWorkno"
                               style="width: 100px;" />
                        <span @dblclick="dbHCompany">
                            {{$t('lang_communication.proFlow.pladCompany')}}：<Input v-model="pladCompanyDis"
                                   :placeholder="$t('lang_communication.proFlow.pladCompanyDis')"
                                   style="width: 100px"
                                   icon="ios-search"
                                   :readonly="true"
                        ...
                        <span @dblclick="clearDepart">
                            {{$t('lang_communication.proFlow.pladDept')}}：<Input v-model="pladDeptDis"
                                   style="width: 100px"
                                   icon="ios-search"
                                   :readonly="true"
                                   :placeholder="$t('lang_communication.proFlow.pladDeptDis')"
                                   @on-click="pickDepart" />
                        </span>
                        {{$t('lang_communication.proFlow.flowCName')}}：<Input :placeholder="$t('lang_communication.proFlow.flowCNameDis')"
                               v-model="flowCName"
                               style="width: 100px;" />
                        <span>
                            ...
                            <Select v-model="stateValue"
                                    style="width: 150px"
                                    :placeholder="$t('lang_communication.pubworkStatus.titleval')"
                                    clearable>
                                <Option :value="ite.key"
                                        v-for="(ite,index) in pubstateList"
                                        :key="index">{{ite.value}}</Option>
                            </Select>
                        </span>
                        <Button type="primary"
                                :loading="isSpin"
    ...
    methods: {
        ...
        deleteOrder (id) {
            ...
            t.$Modal.confirm({
                ...
                onOk: () => {
                    const data = {
                        _mt:  this.$global.mt+'CmutWorkorder.delByIds',
                        logType: '删除工单',
                        delIds: id,
                    }
                    for (const dat in data) {
        ...
        getCommunRecordData (orderId) {
            const t = this
            const data = {
                _mt:  this.$global.mt+'CmutWorkorderrecord.getObject',
                logType: '查看记录',
                sort: 'id',
                order: 'asc',
            }
