<template>
    <div>
        <card :bordered="false">
            <p slot="title"
               class="title">
                <Icon type="stats-bars"></Icon> <span style="margin-left: 5px;">{{$t('lang_homePage.employee.title')}}</span>
            </p>
            <a href="#"
               slot="extra">
                <Icon type="ios-more"
...
export default {
    ...
    methods: {
        getDate () {
            ...
            //     _mt: this.$global.mt + 'AnsrptStatistics.getStaffConditionStatisticForPc',
            // }).then((res) => {
            //     if (isSuccess(res, t)) {
            ...
            //         })
            //         getDataLevelUserLogin({
            //           _mt:  this.$global.mt+'AnsrptStatistics.getStaffEducationStatisticForPc',
            //         }).then((res) => {
            //           if (isSuccess(res, t)) {
            //             const result = res.data.content[0].value
