  ...
  <div :style="{height:oaTodoHeight + 'px'}">
    ...
    <div class="title">
      ...
      <span style="margin-left: 10px;margin-top: 2px"> {{$t('reminder.view')}} {{((page - 1) * rows + 1)}} - {{page*rows > total ? total : page*rows}}  {{$t('reminder.of')}}  <span>{{total}}</span></span><Button size="small" shape="circle" icon="md-refresh" style="margin-left: 20px;display: inline-block;" @click="getData(1)"></Button></Row>
          ...
          <div class="approvIdea" v-if="aprdAprvresult === 'pending'">
          	...
          	<Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="135">
          		<Row>
          			...
          			<i-col span="9" offset="1">
          				<FormItem label="审批时间">
          					 <DatePicker type="datetime" placeholder="请选择审批时间..." disabled style="width: 95%;" v-model="formValidate.aprdAprvtime"></DatePicker>
          				</FormItem>
          			</i-col>
          		</Row>
          		<Row>
          		...
          		<Row>
          			<i-col span="19" offset="2" v-if="!formValidate.aprdAprvopinionFlag">
...
<!--        	</div>-->
        	<div v-if="aprdAprvresult === 'pending'">
...
<script>
  ...
  import testUpdPage from '../../ansrptManage/toDoListManage/commonSinglePageUpdate'
  export default {
    data() {
      return {
...
//    		const t = this
//    		const data = {
//    			_mt:  this.$global.mt+'PlatAutoApprovalEditAgwService.showSubmitButton',
//    			funId: '1',
//    			logType: '按钮显示',
//    			flowId: t.flowId,
...
//    		})
//    	},
      getApvRecordInfo(oaTodoParam) {
        const t = this
        t.oaTodoParam = oaTodoParam
        t.aprdBillid = oaTodoParam.approvBillId
        t.thisPkValue = oaTodoParam.flowDataId
        t.userRole = oaTodoParam.userRole
        getDataLevelUserLogin({
          _mt:  this.$global.mt+'PlatAprvrecord.getApvRecordInfo',
          logType: '查询审批记录',
          aprvRecordid: oaTodoParam.approvRecordId,
        }).then((res) => {
          if (isSuccess(res, t)) {
            // formValidate: {
            //   aprdApprover: this.aprvrelaApproverid,
            ...
            //     aprdAprvopinionFlag: false,
            // },
            t.formValidate.aprdApprover = res.data.content[0].aprdApprover
            t.formValidate.aprdApproverDis = res.data.content[0].aprdApproverDis
            t.formValidate.aprdAprvresult = res.data.content[0].aprdAprvresult
            t.aprdAprvresult = res.data.content[0].aprdAprvresult
            t.formValidate.aprdAprvresultDis = res.data.content[0].aprdAprvresultDis
            t.formValidate.aprdAprvtime = new Date()
            if (t.formValidate.aprdAprvresult === '02noneapproved') {
              this.formValidate.aprdAprvopinionFlag = false
            } else {
              this.formValidate.aprdAprvopinionFlag = true
            }
            t.formValidate.aprdAprvopinion = res.data.content[0].aprdAprvopinion
            t.aprvrelaStepidAll = res.data.content[0].aprdStepid
            t.apblTitlecn = res.data.content[0].apblTitlecn
            this.getAnsrptRecord()
          }
        }).catch(() => {
          this.$Modal.error({
    	...
    	getAnsrptRecord() {
    		...
    		getDataLevelUserLogin({
	          ...
	          if (isSuccess(res, t)) {
                ...
                t.aprvrelaApproverid = t.approvRecordList[0].aprdApprover
                t.aprvrelaRecordidAll = t.approvRecordList[0].id
                t.isHaveRecord = true
                t.getDataBlock()
                t.getSelect()
              }
      ...
      getDataBlock() {
        ...
        let data = {
          ...
          roleType: t.userRole, // 角色类型
          logType: 'getDataBlock3', // 主键值
          pkValue: t.thisPkValue,
          selType: t.isType === 'toDoList' ? '' : 'logTrack',
        }
//        for (const dat in data) {
//          if (data[dat] === undefined) {
//            delete data[dat]
    ...
    * */
    getFormDataSubmit(data) {
      ...
      getColumn(dataBlockId, dataBlockType) {
        const t = this
        const params = {
          _mt:  this.$global.mt+'PlatAutoApprovalEditAgwService.getDataBlockColumn',
...
//      t.submit2()
      ...
      submit2() {
          ...
          if (valid) {
            ...
            const data = {
		      		_mt:  this.$global.mt+'PlatAutoApprovalSubmitAgwService.submit',
		      		logType: '提交审批',
            ...
            getDataLevelUserLogin(data).then((res) => {
              if (isSuccess(res, t)) {
                ...
                t.loading = true
              	t.getApvRecordInfo(t.oaTodoParam)
              }
            }).catch(() => {
              t.$Modal.error({
                title: this.$t('reminder.err'),
      ...
      save2() {
          ...
          if (valid) {
            ...
            data1.aprdAprvtime = new Date(data1.aprdAprvtime).format('yyyy-MM-dd hh:mm:ss')
            const data = {
		      		_mt:  this.$global.mt+'PlatAutoApprovalSaveAgwService.addOrUpd',
		      		logType: '保存审批',
		      		aprvrelaFlowid: t.flowId,
		      		aprdStepid: t.blockStepId,
		      		roleType: t.userRole,
