  ...
  export default{
    ...
    methods: {
      ...
      close() {
        this.countryCname = ''
        this.params.APpage = 1
        if(this.typeName=='基本信息'||this.typeName=='地址信息'||this.typeName=='教育信息'){
          this.$emit('closeUp')
        }
      ...
      dbCkick(row) {
        ...
        }else if(t.typeName=='地址信息'){
          this.$emit('changeinputaddrCoutid', row.countryCname, row.id)
        }else if(t.typeName=='教育信息'){
          this.$emit('changeinputCountryEducation', row.countryCname, row.id)
        }
