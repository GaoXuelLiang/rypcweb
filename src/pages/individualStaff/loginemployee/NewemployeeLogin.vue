    ...
    <div class="login-con"
        ...
        <div class="login-box">
            ...
            <div class="login-right">
                <div class="title">
                    <img src="../../../../static/img/huaxiaLogo.png"
                         alt="">
                </div>
                    ...
                    <Form ref="loginForm"
                        ...
                        <FormItem prop="noteCode"
                            ...
                            <Row class="form_code">
                                ...
                                <span slot="prepend">
                                    <img src="../../../../static/img/icon-valitir.png"
                                         alt="">
                                </span>
                                </Input>
                                </Col>
...
import { setCookie, isSuccessNew, getCookie } from '../../../lib/util'
...
export default {
    ...
    methods: {
        getIndividualTime(){
            ...
            let createTime = url.split('?')[1]
            let finallyTime = Number(createTime)+24*3*60*60*1000
            let nowTime = new Date().getTime()
            if(nowTime>finallyTime){
                t.loseEfficacy = true
                t.$Modal.warning({
                    title: '提示',
                    content: '链接已失效，无法登录',
        ...
        login () {
            ...
            this.$refs.loginForm.validate((valid) => {
                ...
                if (valid) {
                    let obj = {
                        _mt: this.$global.mt + 'UserInfo.login',
                    ...
                    getDataLevelNone(obj).then((res) => {
                        ...
                        if (isSuccessNew(res, t)) {
                            ...
                            localStorage.setItem('userphone',t.form.userName)
                            ...
                            // 保存新员工入职url链接
                            t.entryLoginUrl = res.data.content[0].lastLoginUrl
                            // if(t.entryLoginUrl!=''||t.entryLoginUrl!=undefined){
                            ...
                            if(res.data.content[0].lastLoginUrl){
                                if(res.data.content[0].lastLoginUrl.indexOf('$')>-1){
                                    let routeUrl = res.data.content[0].lastLoginUrl.split('$')[1]
                                    if(routeUrl=='01'){
                                        t.$router.push('/individualStaffMange')
                                        t.$store.commit('setEntryLoginUrl', t.entryLoginUrl.split('#')[1])
                                    }else if(routeUrl=='02'){
                                        t.$router.push('/individualStaffUpload')
                                    }
                                }
                            }else{
                                this.$Modal.warning({
                                    title: this.$t('reminder.err'),
                                    content: '此链接为新员工入职申请表，入职成功无法登录',
                                })
                            }
                        }else {
                            t.form.noteCode = ''
