    ...
    <Row>
      ...
      <card>
        <p slot="title" style="height: 33px;line-height: 33px;">
          <Icon type="ios-bookmark"></Icon>&nbsp;考勤打卡记录
          <!--<Button class="lbtn" v-bind:class="{checked: ischecked2}" @click="TransformationMode($event,2)"><Icon type="calendar"></Icon>转换为日历模式</Button>-->
          <!--<Button class="lbtn" v-bind:class="{checked: ischecked1}" @click="TransformationMode($event,1)"><Icon type="clipboard"></Icon>转换为表格模式</Button>-->
        </p>
        <!-- table表格 日历-->
        <div v-show="calendarModel1">
          ...
          <row class="table-form" ref="table-form">
            ...
            <Table @on-select="selectedtable" @on-select-cancel="selectedtable" @on-select-all="selectedtable" @on-select-all-cancel="selectedtable" @on-sort-change="sortable" size="small" border ref="selection" :columns="columns" :data="data" :height="tableheight"></Table>
          </row>
              ...
              <Row class="table_body_box">
                  <Col span="3" class="table_body" v-for="(days,index1) in calendar_data" :key="index1">
                    ...
                    <div v-if="days.calendarComptype === '01workday' || days.calendarLegaltype === '01workday'" class="conbox area bgWhite">
                      ...
                      <span class="edit" v-if="days.calendarDate" @click="openUp(days.id, $t('button.y.upd'), index1)">{{$t('button.edi')}}</span>
                    </div>
                    <!-- 休息日、节假日样式 -->
