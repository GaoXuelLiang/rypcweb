...
export default {
    ...
    methods: {
        dbCkick (row) {
            const t = this
            t.empCname = ''
            this.$emit('changeinput', row.empCname, row.id, row.empcompHirecompany, row.empcompHirecompanyDis,
                row.empcompDept, row.empcompDeptDis, row.companyId, row.empGid, row.empcompPost)
            this.getData()
            t.close()
        },
        search () {
        ...
        getData () {
            ...
            getDataLevelUserLoginNew(data).then((res) => {
                if (isSuccess(res, t)) {
                    ...
                    this.$emit('apply_ot', res.data.content[0].rows[0])
                }
            }).catch(() => {
                t.$Modal.error({
