  ...
  <div class="background">
    <div class="backbox">
      ...
      <Row class="table-form">
        <Input placeholder="请输入岗位" style="width: 200px" v-model="normName"/>
        <span style="margin: 0;"><Button type="primary"  @click="search" :loading="isSpin">{{$t('button.ser')}}</Button></span>
      </Row>
...
//                },
    ...
    methods: {
      ...
      dbCkick(row) {
        const t = this
        this.$emit('changeinputP', row, row.id)
        t.close()
      },
      sizeChange(size) {
        const t = this
