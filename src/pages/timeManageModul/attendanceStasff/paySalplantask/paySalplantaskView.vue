      ...
      <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="135">
        ...
        <Row>
          ...
          <FormItem :label="$t('lang_payroll.paySalplan.salpltkIskeyDis')" prop="salpltkIskey">
            <RadioGroup v-model="formValidate.salpltkIskey" type="button" size="small" :placeholder="$t('lang_payroll.paySalplan.salpltkIskeyDisCheck')">
    ...
    methods: {
      ...
      getData(id) {
        ...
        getDataLevelUserLogin({
          _mt:  this.$global.mt+'PaySalplantask.getById',
          id: id,
          logType: '根据id获取数据',
        }).then((res) => {
          if (isSuccess(res, t)) {
            t.formValidate = res.data.content[0]
           // t.formValidate.salpltkTime = new Date(t.formValidate.salpltkTime)
          }
        }).catch(() => {
          this.$Modal.error({
      ...
      handleSubmit() {
        ...
        params._mt =  this.$global.mt+'PaySalplantask.addOrUpd'
