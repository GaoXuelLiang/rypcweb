          ...
          <div class="content">
            ...
            <div class="user">
              <Dropdown transfer trigger="hover" @on-click="handleClickUserDropdown">
                <a href="javascript:void(0)" style="display: block;width: 100%;height: 47px;line-height: 47px;">
              ...
              <img :src="cropedImg" style="margin-left: 10px"></img>
            </div>
          </div>
...
export default {
  ...
  methods: {
    handleClickUserDropdown() {
      getDataLevelUserLogin2({ _mt:  this.$global.mt+'UserInfo.logout' }).then((res) => {
    ...
    getTodo() {
      let t = this
      let data = {
        _mt:  this.$global.mt+'CmutNotic.getLoginList',
        logType: '通知公告',
        funId: '1',
      }
    ...
    getNode() {
        const t = this
            getDataLevelUserLogin3({
              _mt:  this.$global.mt+'AuthUserPerm.getEssUserPerm',
              companyId: '1000',
              roleType: '6essmss',
              language: 'CN',
            }).then((res) => {
...
//                 setCookie('useName', t.form.userName)
    ...
    toPage(name) {
      ...
      if (st[1] === '1201') {
          const data = {
            _mt:  this.$global.mt+'ViewEmpMaster.getSubordinatesPage',
            rows: t.rows,
            page: t.page,
            sort: t.sort,
            order: t.order,
...
.main{
  ...
  .head{
    ...
    height: 0;
    padding-top: 588px;
    background: url('../../../../static/employee/bg-02.png') center no-repeat;
