    ...
    <Row>
      ...
      <card>
        ...
        <row class="table-form" ref="table-form">
          <Table @on-select="selectedtable" @on-select-cancel="selectedtable" @on-select-all="selectedtable" @on-select-all-cancel="selectedtable" @on-sort-change="sortable" :height="tableheight" size="small" ref="selection" :columns="columns" :data="data"></Table>
        </row>
    ...
    methods: {
      getData(page) {
        ...
        getDataLevelUserLogin({
          _mt:  this.$global.mt+'ViewEmpMaster.getEmpTotal',
          empId: this.$store.state.user.sysUsempid,
...
//        this.$store.commit('subordinatesPo/setIndex', index)
      ...
      getSelect() {
        const t = this
        const data = {
          _mt:  this.$global.mt+'OrgUnits.getSelectValue',
          unitType: '70dept',
          logType: this.$t('button.ser'),
        }
        getDataLevelUserLoginNew(data).then((res) => {
        ...
        getDataLevelUserLogin({
          _mt:  this.$global.mt+'OrgPost.getSelectValue',
          logType: this.$t('button.ser'),
          data: '{}',
        }).then((res) => {
          if (isSuccess(res, t)) {
