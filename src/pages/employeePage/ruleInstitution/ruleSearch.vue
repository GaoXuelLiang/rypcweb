        ...
        <Row>
          ...
          <Row>
            <Input style="width: 300px;" size="large" v-model="keyWords" placeholder="请输入规章制度关键字..." >
		        	<Button slot="append"  @click="searchRule"></Button>
		    		</Input>
          </Row>
    ...
    methods: {
    	getAllData() {
    		...
    		const data1 = {
				...
				const data = {
          _mt:  this.$global.mt+'CmutKnowledge.getCompanyRules',
         	funId: 1,
...
//	            	res.data.content[0].value[i].cmutKldLosttime = new Date(res.data.content[0].value[i].cmutKldLosttime).format('yyyy-MM-dd')
        ...
        const data = {
          _mt:  this.$global.mt+'CmutKnowledge.getObject',
         	funId: 1,
          logType: this.$t('button.ser'),
          sort: 'id',
        	order: 'desc',
...
//	            	res.data.content[0].value[i].cmutKldLosttime = new Date(res.data.content[0].value[i].cmutKldLosttime).format('yyyy-MM-dd')
        ...
        const data = {
          _mt:  this.$global.mt+'CmutKnlType.getCompanyRules',
          funId: 1,
          logType: this.$t('button.ser'),
      ...
      searchRule() {
      	const t = this
      	const data = {
          _mt:  this.$global.mt+'CmutKnowledge.getByKeyWords',
         	funId: 1,
          logType: this.$t('button.ser'),
          sort: 'id',
        	order: 'desc',
