            ...
            <div class="box">
                ...
                <div class="content"
                    ...
                    <div class="dataBlocks"
                         ...
                         :ref="item.flsdbOrder"
                         :key="item.flsdbOrder"
                         v-if="dbcdHqtype?(index === 0 ? true : (updateForm(item.flsdbOrder) ? true : false)): true">
                        ...
                        <div class="dataContent">
                            <commonSingleForm v-if="item.blockColumn"
                                              ...
                                              :disabled="updateForm(item.flsdbOrder, 'disabled') ? (clStatus === '01hand' ?false: true) : (item.flsdbOptauth === '01view' || disabledForm)"
                                              :ref="'block' + item.id"
                    ...
                    <div class="dataBlocks"
                         ...
                         :key="item.flsdbMark"
                         v-show="dbcdHqtype?(updateForm(item.flsdbOrder) ? true : false) : true">
                        <div class="dataBlocksTitle">
                            <Icon type="compose"
                                  style="margin-right: 5px;"></Icon>{{item.flsdbName}}
                        </div>
                        <div class="dataContent">
                            <!-- <childTable :funId="item.flsdbSubform"
                                        ...
                                        :flsdbSubisupd="item.flsdbSubisupd"> </childTable> -->
                            <childTable :funId="item.flsdbSubform"
                    ...
                    <div class="approvIdea"
                         v-if="aprdAprvresult === '00processing' && inform && showHqtype">
                        <div class="dataBlocksTitle2">
                            <Icon type="compose"
                ...
                <div v-if="inform">
                    <div class="footer"
                        ...
                        <div v-if="!displayHide">
                            <Button @click="closeFlow">取消</Button>
                        </div>
...
export default {
    ...
    props: {
        ...
        dbcdType: String,
        dbcdHqtype: String,
        homePageFiltering: String,//page过滤
        clStatus: String
    },
    mounted () {
        this.init()
    ...
    computed: {
        ...
        showHqtype () {
            let rtn = true
            if (this.dbcdHqtype == '03finance' || this.dbcdHqtype == '04istration' || this.dbcdHqtype == '05person' || this.dbcdHqtype == '06hrbp') {
                rtn = false;
            }
            return rtn;
        },
        updateForm () {
            // if (this.clStatus === '01hand') {
            return (order, type) => {
                let rtn = false;
                if (this.clStatus === '02processed' && type === 'disabled') {
                    rtn = true;
                } else {
                    switch (this.dbcdHqtype) {
                        case '03finance':
                            if (order === '1800') {
                                rtn = true;
                            }
                        ...
                        case '04istration':
                            ...
                            break;
                        case '05person':
                            if (order === '2200' || order === '2100' || order === '2000' || order === '1950' || order === '1975') {
                                rtn = true;
                                if (type === 'disabled' && order === '1975') {
                                    rtn = false;
                                }
                            }
                            break;
                        case '07empid':
                            ...
                            break;
                        case '06hrbp':
                            if (order === '1200' || order === '1300' || order === '1400' || order === '1500' || order === '1600' || order === '1650' || order === '1675' || order === '1700') {
                                rtn = true;
                            }
                            break;
                    }
    ...
    methods: {
        ...
        isAprSubmit () {
            const t = this
            if (t.dbcdHqtype !== '' && t.dbcdHqtype !== undefined && t.dbcdHqtype !== '07empid') {
                t.$Modal.confirm({
                    title: this.$t('reminder.remind'),
                    content: '是否确认提交？',
                    onOk: () => {
        ...
        async aprSubmit () {
            ...
            let a = true
            if (t.dbcdHqtype !== '' && t.dbcdHqtype !== undefined) {
                for (let i = 0; i < this.$children.length; i++) {
                    if (this.$children[i].$options) {
                        if (this.$children[i].$options._componentTag === 'commonSingleForm') {
                            let b = true
                ...
                for (let i in t.formDataSubmit) {
                    if (i == '_mt' || i == 'APlogType') {
                        delete t.formDataSubmit[i]
                    }
                }
                for (let j in t.formDataSubmit) {
                    if (typeof (t.formDataSubmit[j]) !== 'string') {
                        t.formDataSubmit[j] = (t.formDataSubmit[j]).toString()
                    }
                }
                pkValue = t.formDataSubmit
                console.log(t.formDataSubmit, "formDataSubmit")
            } else {

            }
            if (t.dbcdHqtype !== '' && t.dbcdHqtype !== undefined) {
                t.formValidate.aprdAprvresult = '01approved'
            }
