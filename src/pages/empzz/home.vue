...
import empToDoList from "./toDoList";
import empDbchoose from "./dbchoose"; // 竞选平台
import flowDeal from "./flowDeal"; // 流程办理
import myErecord from "./myErecord"; // 我的电子档案
import empBaseInfo from "./empBaseInfo"; // 基本信息
import {
...
export default {
    ...
    methods: {
        ...
        getEmpId () {
            const t = this;
            let data = {
                _mt: this.$global.mt + "EmpInfo.getEmpId",
            };
                ...
                .then((res) => {
                    if (isSuccess(res, t)) {
                        if(res.data.content[0]){
                            ...
                            t.$store.commit('setUserEmpId', data.id);
                            t.$store.commit('setUserEmpName',data.empName)
                        }else{
                            t.$Modal.warning({
                                title:'提示',
                                content: '您无员工权限请联系后台管理员',
                            });
