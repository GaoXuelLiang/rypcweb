        ...
        <Row>
            ...
            <card>
                <p slot="title">
                    ...
                    &nbsp;{{$t('lang_baseManage.baseExrate.title')}}
                </p>
                <Row>
                    <Dropdown>
                        <Button type="primary">
                        ...
                        <DropdownMenu slot="list">
                            ...
                            <span v-for="(item,index) in selectCurrency"
                                  :key="index"
                                  @click="selected(item.id,item.currCurrency)">
                                <DropdownItem>{{item.currCurrency}}</DropdownItem>
                            </span>
                        </DropdownMenu>
                    </Dropdown>
                    <Button type="primary"
...
<script>
import update from './addNewExrate'
...
export default {
    ...
    data () {
        return {
            isSpin: false,
            imp_mt:  this.$global.mt+'BaseExrate.importData',
            openImport: false,
            expDataTital: [{ code: 'exrLocalcurrencyName', name: this.$t('lang_baseManage.baseExrate.exrLocalcurrencyName') }, { code: 'exrCurrencyName', name: this.$t('lang_baseManage.baseExrate.exrCurrencyName') }, { code: 'exrDate', name: this.$t('lang_baseManage.baseExrate.exrDate') },
            { code: 'exrExrate', name: this.$t('lang_baseManage.baseExrate.exrExrate') }, { code: 'comment', name: this.$t('lang_baseManage.baseExrate.comment') }],
            openExpDow: false,
            openExp: false,
            filekey: '',
            filename: '',
            ...
            columns: [
                ...
                {
                    title: this.$t('lang_baseManage.baseExrate.exrLocalcurrencyName'),
                    key: 'exrLocalcurrencyName',
                    sortable: 'custom',
                    //          width: 130,
                },
                {
                    title: this.$t('lang_baseManage.baseExrate.exrCurrencyName'),
                    key: 'exrCurrencyName',
                    sortable: 'custom',
                    //          width: 130,
                },
                {
                    title: this.$t('lang_baseManage.baseExrate.exrDate'),
                    key: 'exrDate',
                    sortable: 'custom',
                    //          width: 130,
                },
                {
                    title: this.$t('lang_baseManage.baseExrate.exrExrate'),
                    key: 'exrExrate',
                    sortable: 'custom',
                    //          width: 130,
                },
    ...
    methods: {
        ...
        getData (page, value) {
            ...
            const data = {
                _mt:  this.$global.mt+'BaseExrate.getPage',
                rows: t.rows,
                page: t.page,
                sort: t.sort,
                order: t.order,
                logType: this.$t('button.ser'),
                exrLocalcurrency: t.exrCurrencyName,
            }
            if (value) {
        ...
        expData () {
            const data = {
                exrCurrency: this.exrCurrencyName,
            }
            this.$refs.expwindow.getData(this.expDataTital, 'baseExrate.export', data)
            this.openExp = true
        },
        closeExp () {
            const t = this
                ...
                t.$Modal.confirm({
                    ...
                    onOk: () => {
                        getDataLevelUserLogin({
                            _mt:  this.$global.mt+'BaseExrate.delByIds',
                            funId: '1',
        ...
        closeUp () {
            const t = this
            t.openUpdate = false
            t.$refs.update.formValidate.exrCurrency = ''
            t.$refs.update.formValidate.exrCurrencyName = ''
            t.$refs.update.formValidate.exrDate = ''
            t.$refs.update.formValidate.exrExrate = ''
