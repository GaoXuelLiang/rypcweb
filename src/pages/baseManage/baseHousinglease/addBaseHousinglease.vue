  ...
  export default {
    data() {
      return {
        ...
        leaseCitysDis: '',
        formValidate: {
          _mt:  this.$global.mt+'BaseHousinglease.addOrUpd',
          funId: '1',
          leaseYear: '',
          leaseAmout: '',
          leaseSdate: '',
...
//        ],
        ruleValidate: {
          ...
          leaseSdate: [
            { required: true, type: 'date', message: this.$t('lang_baseManage.baseHousinglease.r_leaseSdate'), trigger: 'blur' },
          ],
        },
      }
    },
    ...
    methods: {
      getData(id) {
        ...
        getDataLevelUserLogin({
          _mt:  this.$global.mt+'BaseHousinglease.getById',
          id: id,
          logType: '根据id获取数据',
        }).then((res) => {
          if (isSuccess(res, t)) {
            t.formValidate.leaseYear = res.data.content[0].leaseYear
            t.formValidate.leaseCitysDis = res.data.content[0].leaseCitysDis
