<template>
  <div class="table">
    <Row>
      ...
      <card>
        <p slot="title">
          <Icon type="ios-bookmark"></Icon>&nbsp;{{$t('lang_role.cmutnotic.title')}}
        </p>
        <Row>
        ...
        <row class="table-form" ref="table-form">
          <Table @on-select="selectedtable" @on-select-cancel="selectedtable" @on-sort-change="sortable"
                 @on-select-all="selectedtable" @on-select-all-cancel="selectedtable" size="small" border ref="selection" :columns="columns" :data="comInfo"
...
//        t.getData(1)
      ...
      deletemsg() {
        ...
        if (t.updateArr.length === 0) {
          ...
          t.$Modal.confirm({
            ...
            onOk: () => {
              const data = {
                _mt:  this.$global.mt+'CmutNotic.delByIds',
      ...
      modifystatu(state) {
        ...
        getDataLevelUserLogin({
          _mt:  this.$global.mt+'CmutNotic.setNoticesInvalid',
          logType: logType,
          noticeState: state,
          ids: t.selectedArr,
        }).then((res) => {
