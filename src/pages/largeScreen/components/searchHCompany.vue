    ...
    <div class="backbox">
      <div class="title">
        <div class="title-text">
          <Icon type="ios-bookmark" size="16"></Icon>&nbsp;{{$t('lang_employee.searchColumn.searchCompany')}}
        </div>
        <img class="btn_img" @click="close" src="../../../../static/largeScreen/popup_close.png" alt="">
      </div>
      <Row class="table-form">
        <Input :placeholder="$t('lang_employee.searchColumn.hireCompanyDis')" style="width: 200px" v-model="unitsName"/>
      ...
      <row class="table-form" ref="table-form">
        <Table height="320" size="small" ref="selection" :columns="searchHCClo" @on-sort-change="sortable" :data="data" @on-row-dblclick="dbCkick"></Table>
      </row>
    ...
    methods: {
      search() {
    	...
    	dbCkick(row) {
    		const t = this
        this.close()
	        t.$emit('inputHCompany', row.unitsName, row.id, row.unitsSname)
	        t.$emit('closeHCompany')
    	},
    	sortable(column) {
