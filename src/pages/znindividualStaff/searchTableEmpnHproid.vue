...
//          data: '{"cityType":"02city","cityIsvalid" : "1"}',
    ...
    methods: {
      ...
      getData(name,type,cityType,cityPid) {
        const t = this
        t.isSpinTable = true
        t.loading = true
        if(name&&type){
      ...
      dbCkick(row) {
        ...
        if(t.title == '户口所在省'){
            this.$emit('changeinputEmpnHproid', row)
             ...
             this.$emit('changeinputaddrProid1', row)
        }else if(t.title == '户籍所在省'){
             this.$emit('changeinputaddrProid', row)
        }else if(t.title == '户籍所在市'){
            this.$emit('changeinputaddrCityid', row)
        }else if(t.title == '现居住地所在市'){
            this.$emit('changeinputaddrCityid1', row)
        }else if(t.title == '现居住地所在区县'){
            this.$emit('changeinputaddrCouid1', row)
