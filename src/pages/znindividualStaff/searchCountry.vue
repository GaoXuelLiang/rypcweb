  ...
  export default{
    ...
    methods: {
      ...
      close() {
        this.countryCname = ''
        this.params.APpage = 1
        if(this.typeName=='基本信息'||this.typeName=='地址信息1'||this.typeName=='教育信息'||this.typeName=='地址信息2'){
          this.$emit('closeUp')
        }
      ...
      dbCkick(row) {
        const t = this
        if(t.typeName=='基本信息'){
          this.$emit('changeinputCountry', row.coutCname, row.id)
        }else if(t.typeName=='地址信息1'){
          this.$emit('changeinputaddrCoutid', row.coutCname, row.id)
        }else if(t.typeName=='教育信息'){
          this.$emit('changeinputCountryEducation', row.coutCname, row.id)
        }else if(t.typeName=='地址信息2'){
          this.$emit('changeinputaddrCoutid2', row.coutCname, row.id)
        }
