<template>
  <div class="background docVar">
    <div class="backbox">
      <div class="title">
        <div class="title-text">
          <Icon type="ios-bookmark" size="16"></Icon>
          {{$t('lang_flow.varList.title')}}
        </div>
  ...
  <div class="table-form">
    ...
    <row class="table-form" ref="table-form">
      <Table stripe :loading="loading" :height="childTableHeight" :no-data-text="noDataImg" @on-selection-change="selectedtable" @on-sort-change="sortable" size="small" ref="selection" :columns="columns" :data="$store.state.flowStepData.childTable4"></Table>
    </row>
    ...
    <updDocVar v-show="showMsg" @hideMsg="hideMsg" ref="updDocVar" :logType="logType"></updDocVar>
    </div>
  </div>
</template>
<script>
  import updDocVar from './updDocVar'
    ...
    methods: {
      getdata() {
        ...
        const data = {
          ...
          flspvarSource: 'flowdoc',
          APflowId: t.$store.state.flowStepData.flowId,
          flspvarBillid: t.$store.state.flowStepData.childId4,
        }
...
//      将本条数据的id传入mainid（如果有的话）
        ...
        if (id) {
          t.$refs.updDocVar.getFormData(id)
        }
      },
      hideMsg() {
        this.showMsg = false
