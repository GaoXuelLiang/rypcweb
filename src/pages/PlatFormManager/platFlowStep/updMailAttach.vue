...
                  v-if="spinShow"></Spin>
      ...
      <Row class="content">
        <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" label-position="right" :label-width="135">
          ...
          <i-col span="10"  offset="1" v-show=" typeValue === '01fixed'">
            <FormItem :label="$t('lang_flow.mailAttach.flsmlatAttach')"  prop="flsmlatAttach">
              <span @dblclick="dbclear">
                <Input v-model="flsmlatAttachDis"  icon="ios-search" :placeholder="$t('lang_flow.mailAttach.p_flsmlatAttach')" @on-click="pickData"/>
            </span>
            </FormItem>
          </i-col>
          <i-col span="10" offset="1" v-show=" typeValue === '03flowgen'">
            <FormItem :label="$t('lang_flow.mailAttach.flsmlatGendocid')" prop="flsmlatGendocid">
              <span @dblclick="dbclear">
                <Input v-model="flsmlatGendocidDis"  icon="ios-search" :placeholder="$t('lang_flow.mailAttach.p_flsmlatGendocid')" @on-click="pickData"/>
            </span>
            </FormItem>
          </i-col>
          <i-col span="10" offset="1" v-show=" typeValue === '02flowupload'">
            <FormItem :label="$t('lang_flow.mailAttach.flsmlatUpdfield')"  prop="flsmlatUpdfield">
              <span @dblclick="dbclear">
                <Input v-model="flsmlatUpdidDis"   :placeholder="$t('lang_flow.mailAttach.p_flsmlatUpdfield')" @on-click="pickData2"/>
            </span>
            </FormItem>
          </i-col>
          <i-col span="21" offset="1">
            <FormItem :label="$t('lang_flow.mailAttach.flsmlatConditionDis')" prop="flsmlatConditionDis">
              <Input v-model="formValidate.flsmlatConditionDis" type="textarea" readonly="readonly" :autosize="{minRows: 2,maxRows: 5}" placeholder="请设置附件条件..." class="settlement"></Input>
          ...
          <i-col span="10" offset="1">
            <FormItem :label="$t('lang_flow.mailAttach.flsmlatIsvalid')"  prop="flsmlatIsvalid">
              <RadioGroup  v-model="formValidate.flsmlatIsvalid" type="button" size="small">
                <Radio :label="item.paramCode" v-for="(item,index) in yesOrNo" :key="index">{{item.paramInfoCn}}</Radio>
              </RadioGroup>
            </FormItem>
          </i-col>
          <i-col span="21" offset="1" v-show="formValidate.flsmlatIsvalid === '0'?true:false">
...
<script>
  ...
  //      id: '',
        ...
        transmitValue: '',
        formValidate: {
          _mt:  this.$global.mt+'PlatFlsmlAttach.addOrUpd',
          flsmlatOrder: '',
          flsmlatSource: '',
          flsmlatAttach: '',
          flsmlatGendocid: '',
...
//          logType: '',
        ...
        params2: {
          ...
          APflowId: this.$store.state.flowStepData.flowId,
        },
      }
    },
    props: {
    ...
    methods: {
      ...
      getFormData(id) {
        ...
        getDataLevelUserLoginData({
          _mt:  this.$global.mt+'PlatFlsmlAttach.getById',
          APid: id,
          ...
          if (isSuccess(res, t)) {
            ...
            t.typeValue = res.data.content[0].flsmlatSource
            ...
            t.flsmlatUpdidDis= res.data.content[0].flsmlatUpdfield
      ...
      getSelect() {
        ...
        getDataLevelUserLoginData({
          ...
          APtypeCode: 'yesno,mailattachtype',
        }).then((res) => {
          if (isSuccess(res, t)) {
      ...
      setDocType(value) {
        ...
        if (value === '02flowupload') {
          this.params.flsdcType = '02flowupload'
          this.formValidate.flsmlatAttach = ''
          this.flsmlatAttachDis = ''
          this.formValidate.flsmlatGendocid = ''
          this.flsmlatGendocidDis = ''
      ...
      save() {
        ...
        data.flsmlatUpdfield =this.flsmlatUpdidDis
