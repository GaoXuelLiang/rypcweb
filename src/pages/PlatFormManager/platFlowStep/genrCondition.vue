<template>
    <div class="cover noButton">
        <div class="box">
          ...
          03getbyfield  02filed   01sys

          如果是01sys ，就根据 flvarRtndatafmt这个字段判断，如果是 01number 就是文本框

          02string也是文本框

          03date就是日期款

          flvarValresource是 02 03时就根据layout布局
        -->
                <i-col span="24"
                       class="condition">
                    <Row v-if="flvarValresource === '03getbyfield' || flvarValresource === '02filed'">
                        <Row :gutter="10">
                            <i-col span="6">
                                <Input v-model="selectVariable"
                                       placeholder="请选择变量..."
                                       icon="ios-search"
                                       :readonly="true"
                                       @on-click="clickVariable" />
                            </i-col>
                            <i-col span="6">
                                <Select v-model="logOperatorValue"
                                        >
                                    <Option label="等于"
                                            value="等于"></Option>
                                    <Option label="不等于"
                            ...
                            <i-col span="6">
                                ...
                                <Input v-model="paramterValue"
                                       placeholder="请输入参数值..."
                                       v-if="flvarFieldType === 'p_layout_01'" /> -->
                                <!-- <Select v-model="paramterValue"
                                        placeholder="请选择参数值..."
                                        v-if="flvarFieldType === 'p_layout_12'"
                                        :label-in-value="true"
                                        @on-change="v =>{setOption(v,'type')}">
                                    <Option :value="item.key"
                                            v-for="(item,index) in selectParamVal"
                                            :key="index">{{item.value}}</Option>
                                </Select> -->
                                <!-- <DatePicker type="date"
                                            placeholder="请选择时间..."
                                            v-model="paramterValue"
                                            style="width: 100%"
                                            v-if="flvarFieldType === 'p_layout_03'"></DatePicker>
                                <Input v-model="paramterValue"
                                       icon="ios-search"
                                       :readonly="true"
                                       placeholder="请设置参数值..."
                                       v-if="flvarFieldType === 'p_layout_13'"
                                       @on-click="selectTKVal" /> -->
                            </i-col>
                            <i-col span="6"
                                ...
                                <RadioGroup v-model="paramterValue" type="button" size="small"
                                            @on-change="radioChang(paramterValue)"
                          ...
                          <Button icon="md-refresh" @click="sureUpd">确定</Button>
                      </i-col>-->
                        </Row>
                    </Row>
                    <Row v-if="flvarValresource === '01sys'">
                        <Row :gutter="10">
                            <i-col span="6">
                                <Input v-model="selectVariable"
                                       placeholder="请选择变量..."
                            ...
                            <i-col span="6">
                                ...
                                <Input v-model="paramterValue"
                                       placeholder="请输入参数值..."
                                       v-if="flvarRtndatafmt === '02string' || flvarRtndatafmt === '01number'" />
                                <Select v-model="paramterValue"
                                        placeholder="请选择参数值..."
                                        v-if="flvarFieldType === ''"
                                        :label-in-value="true"
                                ...
                                <DatePicker type="date"
                                            ...
                                            v-model="paramterValue"
                                            style="width: 100%"
                                            v-if="flvarRtndatafmt === '03date'"></DatePicker>
                                <!--<Input v-model="paramterValue"  icon="ios-search" :readonly="true" placeholder="请设置参数值..."  @on-click="selectTKVal"/>-->
                            </i-col>
                            <i-col span="6"
                                   style="line-height: 30px;">
                                <RadioGroup v-model="logOperatorType">
                        ...
                        <i-col span="3"
                            ...
                            <Row v-show="!showUpdate">
                                <Button icon="plus-round"
                                        type="primary"
                                        @click="addCondition">添加</Button>
                            </Row>
                            <Row v-show="showUpdate">
                                <Button icon="plus-round"
                                        type="primary"
                                        @click="sureUpd">添加</Button>
                            </Row>
                            <Row>
                                <Button icon="compose"
                                        type="success"
                                        @click="updateCondition">修改</Button>
                            </Row>
                            <Row>
                                <Button  
                                        type="error"
                                        @click="delArrElement">删除</Button>
                            </Row>
                            <Row>
                                <Button type="primary"
                                        @click="addBracket">( 添加 )</Button>
                            </Row>
                            <Row>
                                <Button type="error"
                                        @click="delBracket">( 去掉 )</Button>
                            </Row>
                            <Row>
                                <Button icon="android-document"
                                        type="primary"
                                        @click="submit">{{$t('button.sav')}}</Button>
                            </Row>
                            <Row>
                                <Button icon="gear-a"
                                        type="primary"
                                        @click="dataCheck">校验</Button>
                            </Row>
                        </i-col>
                    </Row>
                </i-col>
        ...
        <transition name="fade">
            <searchVar v-show="openVar"
                       :searchCloumns="searchCloumns"
                       :params="params"
                       searchTitle="选择变量"
                       @closeUp="closeVar"
                       @changeinput="changeinput"
                       ref="searchVar"></searchVar>
        </transition>
...
import autoSearchtable from '../../common/searchTable/autoSearchtable.vue'
export default {
    data () {
        return {
            loading: false,
    ...
    methods: {
        ...
        async getData (transmitName, transmitValue) {
            ...
            if ((transmitValue.indexOf('&&') === -1) && (transmitValue.indexOf('||') === -1)) {
                ...
                await getDataLevelUserLoginData(data).then((res) => {
                    if (isSuccess(res, t)) {
                        if(!res.data.content[0].rows[0]) {
                            return;
                        }
                        const data2 = {
                        ...
                        getDataLevelUserLoginData({
                            _mt:  this.$global.mt+'PlatFlowVar.getByFlowIdAndMark',
                            APfunId: '1',
                            APlogType: this.$t('button.ser'),
                            APdata: JSON.stringify(data2),//特殊data
                        }).then((res) => {
                            if (isSuccess(res, t)) {
                                t.data.push({
        ...
        changeinput (name, id, operType, valType, varMark) {
            console.log(name, operType, valType, varMark)
            const t = this;
            // t.loading = true;
            //  		变量名
            ...
            t.flvarValresource = '02filed';

            // t.flvarFieldType = 'p_layout_03';
            // const data = {
            //     flvarFlow: t.flowID,
            //     flvarMark: t.variableValue,
            // }
            // if (t.flvarValresource === '01sys') {
            //             t.flvarRtndatafmt = res.data.content[0].flvarRtndatafmt
            // }
            // getDataLevelUserLoginData({
            //     _mt:  this.$global.mt+'PlatFlowVar.getByFlowIdAndMark',
            //     APfunId: '1',
            ...
            //         t.modiaContent = res.data.content[0].popId
            //         t.flvarValresource = res.data.content[0].flvarValresource
            //         if (t.flvarValresource === '01sys') {
            //             t.flvarRtndatafmt = res.data.content[0].flvarRtndatafmt
            //         }
            //     }
        ...
        addCondition () {
            ...
            if (t.selectVariable && t.logOperatorValue && t.paramterValue) {
                if (t.flvarFieldType === 'p_layout_03') {
                ...
                } else if (t.flvarFieldType === 'p_layout_12' || t.flvarFieldType === 'p_layout_11') { // 下拉选择，或者单选框
                    t.data.push({
                    ...
                    if (t.flvarRtndatafmt === '03date' && t.flvarValresource === '01sys') {
                        t.paramterValue = new Date(t.paramterValue).format('yyyy-MM-dd')
                        t.data.push({
                            conditionValue: '"' + t.selectVariable + '"' + t.logOperatorValue + '"' + t.paramterValue + '"' + t.logOperatorType,
                            sendValue: '"' + t.variableValue + '"' + t.sendParValue + '"' + t.paramterValue + '"' + t.sendOperTypeValue,
        ...
        sureUpd () {
            ...
            } else if (t.flvarFieldType === 'p_layout_12' || t.flvarFieldType === 'p_layout_11') {
        ...
        // 单选框
        radioChang (paramterValue) {
            if (paramterValue === '1') {
                this.paramterValue2 = '是'
            } else {
                this.paramterValue2 = '否'
            }
        },
        //  	校验
        dataCheck () {
