...
<script>
  ...
  import render from "../../../../components/vueOrgTree/node";
...
/*        let connections = this.rjsp.getConnections({ source: info.sourceId, target: info.targetId })
      ...
      textOver(info) {
    //     console.log(info)
     //   console.log(info.canvas.childNodes[0])
     //   console.log(info.canvas.nextElementSibling.id)
//        let div1 = document.createElement('div')
        document.getElementById(info.canvas.nextElementSibling.id).innerHTML = '成功'
        document.getElementById(info.canvas.nextElementSibling.id).style.color = 'black'
      },
      textOut(info) {
        document.getElementById(info.canvas.nextElementSibling.id).innerHTML = ''
        document.getElementById(info.canvas.nextElementSibling.id).style.color = ''
      },
      closeLine() {
        this.showLine = false
      },
...
//        this.reload()
      ...
      _addEdge(info, id) {
        Common.events = { dblclick: this.editLabelText, contextmenu: this.deleteLine }
        const c = this.rjsp.connect({ source: info.sourceId, target: info.targetId, overlays: [['Label', { label: '', id: id, location: 0.15 }]] }, Common)
        c.edgeId = id
        this.edges.push(c)
      ...
      _initEdges(edges) {
        const t = this
   //     Common.events = { dblclick: this.editLabelText, mouseover: this.textOver, mouseout: this.textOut, contextmenu: this.deleteLine }
        Common.events = { dblclick: this.editLabelText, contextmenu: this.deleteLine }
        edges.map(edge => {
          edge.overlays = [['Label', { label: edge.labelText, id: edge.id, location: 0.5 }]]
      ...
      * */
      ...
      getListByIdEdge(id) {
        const t = this
        const data = {
          _mt:  this.$global.mt+'PlatFlsRelation.getByFlowId',
          logType: this.$t('button.ser'),
          flsrelFlow: id,
        }
        getDataLevelUserLoginNew(data).then((res) => {
          if (isSuccess(res, t)) {
            ...
            t.lineArray = res.data.content[0].value.map(node => {
              let labelText = node.flsrelConditiontitle
              if (!labelText) {
                labelText = ''
              }
              return {
                ...
                labelText: node.flsrelConditiontitle,
              }
            })
...
//            })
      ...
      * */
      saveAll() {
        ...
        const data = {
          _mt:  this.$global.mt+'PlatFlowstep.updNodeInfo',
          logType: '保存',
      ...
      selectNode(node, index) {
        let t = this
        let data = {
          _mt:  this.$global.mt+'PlatAprvstep.delCheck',
          logType: '刪除校验',
          id: node.storeId,
        }
