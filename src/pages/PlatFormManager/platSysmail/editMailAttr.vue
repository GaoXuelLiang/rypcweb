...
                  v-if="spinShow"></Spin>
            ...
            <Row>
                <Form :model="form"
                    ...
                    <i-col span="10" offset="1">
                        <FormItem :label="$t('lang_platdoc.platMailAttach.sysmlatOrder')"
                                  v-show="showDesc"
                                  prop="sysmlatOrder">
                            <Input v-model="form.sysmlatOrder"
                                   :placeholder="$t('lang_platdoc.platMailAttach.sysmlatOrderDis')"></Input>
                        </FormItem>
                    </i-col>
                    <i-col span="10"
                           offset="1">
                        <FormItem :label="$t('lang_platdoc.platMailAttach.sysmlatSource')"
                                  v-show="showDesc">
                            <Select v-model="form.sysmlatSource"
                                    :placeholder="$t('lang_platdoc.platMailAttach.sysmlatSourceDis')"
                                    :transfer="true"
                                    @on-change="selChangeSource">
                                <Option :value="item.paramCode"
                                        v-for="(item,index) in sysmlatSourceType"
                                        :key="index">{{item.paramInfoCn}}</Option>
                            </Select>
                        </FormItem>
                    </i-col>
                    <i-col span="10" offset="1">
                        <FormItem :label="$t('lang_platdoc.platMailAttach.sysmlatAttach')"
                                  v-show="showAttrHide0">
                            <Input v-model="sysmlatAttachDis"
                                   icon="ios-search"
                                   :readonly="true"
                                   :placeholder="$t('lang_platdoc.platMailAttach.sysmlatAttachDis')"
                                   @on-click="pickData1" />
                            <!--<Input v-model="form.sysmlatAttach" placeholder="请输入固定附件" ></Input>-->
                        </FormItem>
                    </i-col>
                    <i-col span="10" offset="1">
                        <FormItem :label="$t('lang_platdoc.platMailAttach.sysmlatFlowattach')"
                                  v-show="showAttrHide1">
                            <Input v-model="sysmlatFlowattachDis"
                                   icon="ios-search"
                                   :readonly="true"
                                   :placeholder="$t('lang_platdoc.platMailAttach.sysmlatFlowattachDis')"
                                   @on-click="pickData2" />
                            <!--<Input v-model="form.sysmlatFlowattach" placeholder="请输入流程固定附件" ></Input>-->
                        </FormItem>
                    </i-col>
                    <i-col span="10" offset="1">
                        <FormItem :label="$t('lang_platdoc.platMailAttach.sysmlatFlowupdfiled')"
                                  v-show="showAttrHide2">
                            <Input v-model="sysmlatFlowupdfiledDis"
                                   icon="ios-search"
                                   :readonly="true"
                                   :placeholder="$t('lang_platdoc.platMailAttach.sysmlatFlowupdfiledDis')"
                                   @on-click="pickData3" />
                            <!--<Input v-model="form.sysmlatFlowupdfiled" placeholder="请输入流程上传文件" ></Input>-->
                        </FormItem>
                    </i-col>
                    <i-col span="10" offset="1">
                        <FormItem :label="$t('lang_platdoc.platMailAttach.sysmlatFlowgendocid')"
                                  v-show="showAttrHide3">
                            <Input v-model="sysmlatFlowgendocidDis"
                                   icon="ios-search"
                                   :readonly="true"
                                   :placeholder="$t('lang_platdoc.platMailAttach.sysmlatFlowgendocidDis')"
                                   @on-click="pickData4" />
                    ...
                    <i-col span="10"
                        ...
                        <FormItem :label="$t('lang_platdoc.platMailAttach.sysmlatCondition')">
                            <Input v-model="form.sysmlatCondition"
                                   :placeholder="$t('lang_platdoc.platMailAttach.sysmlatConditionDis')"></Input>
                        </FormItem>
                    </i-col>
                    <i-col span="10"
                           offset="1">
                        <FormItem :label="$t('lang_platdoc.platMailAttach.sysmlatIsvalid')">
                            <RadioGroup v-model="form.sysmlatIsvalid" type="button" size="small"
                                        :placeholder="$t('lang_platdoc.platMailAttach.sysmlatIsvalidDis')"
                                        @on-change="validChange()">
                                <Radio :label="item.paramCode"
                                       v-for="(item,index) in yesAndNoType"
                    ...
                    <i-col span="21" offset="1"
                           v-show="openInValid">
                        <FormItem :label="$t('lang_platdoc.platMailAttach.sysmlatInvreason')"
                                  prop="comment">
                            <Input v-model="form.sysmlatInvreason"
                                   type="textarea"
                                   :autosize="{minRows: 2,maxRows: 5}"
                                   :placeholder="$t('lang_platdoc.platMailAttach.sysmlatInvreasonDis')"></Input>
                        </FormItem>
                    </i-col>
                    <i-col span="21" offset="1">
                        <FormItem :label="$t('lang_platdoc.platMailAttach.sysmlatComment')"
                                  prop="comment">
                            <Input v-model="form.sysmlatComment"
                                   type="textarea"
                                   :autosize="{minRows: 2,maxRows: 5}"
                                   :placeholder="$t('lang_platdoc.platMailAttach.sysmlatCommentDis')"></Input>
                        </FormItem>
                    </i-col>
                    <i-col span="21" offset="1">
                        <row type="flex"
        ...
        <transition name="fade">
            <searchAttach v-show="openAttach"
                          @closeAttach="closeAttach"
                          @changeinput="changeinput"
                          ref="searchAttach"></searchAttach>
        </transition>
        <transition name="fade">
            <searchFlowattach v-show="openAttach2"
                              @closeAttach="closeAttach2"
                              @changeinput="changeinput2"
                              ref="searchFlowattach"></searchFlowattach>
        </transition>
        <transition name="fade">
            <searchFlowupdfiled v-show="openAttach3"
                                @closeAttach="closeAttach3"
                                @changeinput="changeinput3"
                                ref="searchFlowupdfiled"></searchFlowupdfiled>
        </transition>
        <transition name="fade">
            <searchFlowgendocid v-show="openAttach4"
                                @closeAttach="closeAttach4"
                                @changeinput="changeinput4"
                                ref="searchFlowgendocid"></searchFlowgendocid>
        </transition>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        getdata (rowId) {
            ...
            t.spinShow = true;
            const params = {
                _mt:  this.$global.mt+'PlatSysmailAttachment.getById',
                id: rowId,
                logType: '查询List信息',
            }
        ...
        save () {
            ...
            params._mt =  this.$global.mt+'PlatSysmailAttachment.addOrUpd'
