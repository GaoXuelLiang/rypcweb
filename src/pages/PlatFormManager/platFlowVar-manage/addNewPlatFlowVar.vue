<template>
    <div class="cover">
            ...
            <Form ref="formValidate"
                ...
                <Row>
                    <i-col span="10" offset="1">
                        <FormItem :label="$t('lang_platform.platFlowVar.flvarFormtype')"
                                  prop="flvarFormtype">
                            <Select v-model="formValidate.flvarFormtype"
                                    :placeholder="$t('lang_platform.platFlowVar.flvarFormtypeIns')">
                    ...
                    <i-col span="10" offset="1">
                        <FormItem :label="$t('lang_platform.platFlowVar.flvarFlow')"
                                  prop="flvarFlow"
                                  v-show="formValidate.flvarFormtype === '01system'">
                            <span @dblclick="clearFlow">
                                <Input v-model="flvarFlowDis"
                                       icon="ios-search"
                                       :readonly="true"
                    ...
                    <i-col span="10" offset="1">
                        <FormItem :label="$t('lang_platform.platFlowVar.flvarMark')"
                                  prop="flvarMark">
                            <Input v-model="formValidate.flvarMark"
                                   :placeholder="$t('lang_platform.platFlowVar.flvarMarkIns')"></Input>
                        </FormItem>
                    </i-col>
                    <i-col span="10" offset="1">
                        <FormItem :label="$t('lang_platform.platFlowVar.flvarName')"
                                  prop="flvarName">
                            <Input v-model="formValidate.flvarName"
                                   :placeholder="$t('lang_platform.platFlowVar.flvarNameIns')"></Input>
                        </FormItem>
                    </i-col>
                    <i-col span="10" offset="1">
                        <FormItem :label="$t('lang_platform.platFlowVar.flvarValresource')"
                                  prop="flvarValresource">
                            <Select v-model="formValidate.flvarValresource"
                                    :placeholder="$t('lang_platform.platFlowVar.flvarValresourceIns')">
                                <Option :value="item.paramCode"
                                        v-for="(item,index) in selectValresource"
                                        :key="index">{{item.paramInfoCn}}</Option>
                            </Select>
                        </FormItem>
                    </i-col>
                    <i-col span="10" offset="1">
                        <FormItem :label="$t('lang_platform.platFlowVar.flvarSysval')"
                                  prop="flvarSysval"
                                  v-if="formValidate.flvarValresource === '01sys'">
                            <Select v-model="formValidate.flvarSysval"
                                    :placeholder="$t('lang_platform.platFlowVar.flvarSysvalIns')">
                                <Option :value="item.id"
                                        v-for="(item,index) in selectFlvarSysval"
                                        :key="index">{{item.platVarCode}}</Option>
                            </Select>
                        </FormItem>
                    </i-col>
                    <i-col span="10" offset="1">
                        <FormItem :label="$t('lang_platform.platFlowVar.flvarFiled')"
                                  prop="flvarFiledDis"
                                  v-if="formValidate.flvarValresource === '02filed' || formValidate.flvarValresource === '03getbyfield'">
                            <span @dblclick="clearFiled">
                                <Input v-model="flvarFiledDis"
                                       icon="ios-search"
                                       :readonly="true"
                                       :placeholder="$t('lang_platform.platFlowVar.flvarFiledIns')"
                                       @on-click="pickData1" />
                            </span>
                        </FormItem>
                    </i-col>
                    <i-col span="10" offset="1">
                        <FormItem :label="$t('lang_platform.platFlowVar.flvarGetvalbyfiled')"
                                  prop="flvarGetvalbyfiledDis"
                                  v-if="formValidate.flvarValresource === '03getbyfield'">
                            <span @dblclick="clearValbyfiled">
                                <Input v-model="flvarGetvalbyfiledDis"
                                       icon="ios-search"
                                       :readonly="true"
                                       :placeholder="$t('lang_platform.platFlowVar.flvarGetvalbyfiledIns')"
                                       @on-click="pickData2" />
                            </span>
                        </FormItem>
                    </i-col>
                    <i-col span="10" offset="1">
                        <FormItem :label="$t('lang_platform.platFlowVar.flvarRtndatafmt')"
                                  prop="flvarRtndatafmt">
                            <Select v-model="formValidate.flvarRtndatafmt"
                                    :placeholder="$t('lang_platform.platFlowVar.flvarRtndatafmtIns')">
                                <Option :value="item.paramCode"
                                        v-for="(item,index) in selectFlvarRtndatafmt"
                                        :key="index">{{item.paramInfoCn}}</Option>
                            </Select>
                        </FormItem>
                    </i-col>
                    <i-col span="10" offset="1">
                        <FormItem :label="$t('lang_platform.platFlowVar.flvarDatefmt')"
                                  prop="flvarDatefmt"
                                  v-if="formValidate.flvarRtndatafmt === '03date'">
                            <Select v-model="formValidate.flvarDatefmt"
                                    :placeholder="$t('lang_platform.platFlowVar.flvarDatefmtIns')">
                                <Option :value="item.paramCode"
                                        v-for="(item,index) in selectFlvarDatefmt"
                                        :key="index">{{item.paramInfoCn}}</Option>
                            </Select>
                        </FormItem>
                    </i-col>
                    <i-col span="10" offset="1">
                        <FormItem :label="$t('lang_platform.platFlowVar.flvarIscondition')"
                                  prop="flvarIscondition">
                            <RadioGroup v-model="formValidate.flvarIscondition" type="button" size="small"
                                        :placeholder="$t('lang_platform.platFlowVar.flvarIsconditionIns')">
                    ...
                    <i-col span="10" offset="1">
                        <FormItem :label="$t('lang_platform.platFlowVar.flvarIsleavecal')"
                                  prop="flvarIsleavecal">
                            <RadioGroup v-model="formValidate.flvarIsleavecal" type="button" size="small">
                                <Radio :label="item.paramCode"
                                       v-for="(item,index) in selectYesNo"
                                       :key="index">{{item.paramInfoCn}}</Radio>
                            </RadioGroup>
                    ...
                    <i-col span="21" offset="1">
                        <FormItem :label="$t('lang_platform.platFlowVar.comment')"
                                  prop="comment">
                            <Input v-model="formValidate.comment"
                                   type="textarea"
                                   :autosize="{minRows: 2,maxRows: 5}"
                                   :placeholder="$t('lang_platform.platFlowVar.commentIns')"></Input>
                        </FormItem>
                    </i-col>
                    <i-col span="21" offset="1">
                        <Row type="flex"
        ...
        <transition name="fade">
            <searchFlvarFiled v-show="openPick1"
                              :title="title"
                              ...
                              ref="searchFlvarFiled"></searchFlvarFiled>
        </transition>
        <transition name="fade">
            <searchValbyfiled v-show="openPick2"
                              :searchCloumns="searchCloumns2"
                              :params="params2"
                              @closeUp="close2"
                              @changeinput="changeinput2"
                              ref="searchValbyfiled"></searchValbyfiled>
        </transition>
    </div>
</template>
<script>
...
export default {
    data () {
        return {
            ...
            updateImg: false,
            formValidate: {
                _mt:  this.$global.mt+'PlatFlowVar.addOrUpd',
                funId: '1',
                flvarMark: '',
    ...
    methods: {
        getData (id) {
            ...
            getDataLevelUserLogin({
                _mt:  this.$global.mt+'PlatFlowVar.getById',
                id: id,
                logType: '根据id获取数据',
            }).then((res) => {
                if (isSuccess(res, t)) {
                    ...
                    t.formValidate.flvarIsleavecal = res.data.content[0].flvarIsleavecal
                    t.flvarIsleavecalDis = res.data.content[0].flvarIsleavecalDis
        ...
        handleSubmit () {
            ...
            if (t.formValidate.flvarFormtype === '01system') {
                t.ruleValidate.flvarFlow = [
                    { required: true, message: this.$t('lang_platform.platFlowVar.flvarFlowIns'), trigger: 'blur' },
                ]
            } else {
                delete t.ruleValidate.flvarFlow
            }
        ...
        getSelect () {
            ...
            getDataLevelUserLogin({
                _mt:  this.$global.mt+'PlatFormVar.getSelectValue',
                data: '{}',
            }).then((res) => {
                if (isSuccess(res, t)) {
