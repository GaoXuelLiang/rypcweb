          ...
          <Icon type="md-close" size="20"></Icon>
			...
			<Row style="overflow-y: auto;">
				<Form :model="form" label-position="right" :rules="ruleValidate"  ref="form" :label-width="135">
					...
					<i-col span="10" offset="1" v-if="$store.state.atsk.atskdfMailvar">
						<FormItem :label="$t('lang_platform.autoTask.atskdfMailvarName')" >
							<span @dblclick="dbMailVar">
							<Input  readonly="readonly" :placeholder="$t('lang_platform.autoTask.atskdfMailvarNameIns')" v-model="atskdfMailvarName" @on-click="selectMailVar" />
							</span>
						</FormItem>
					</i-col>
					<i-col span="10" offset="1" v-if="$store.state.atsk.atskdfSurveyvar">
						<FormItem :label="$t('lang_platform.autoTask.atskdfSurveyvarName')" >
							<span @dblclick="dbSurVar">
							<Input  readonly="readonly" :placeholder="$t('lang_platform.autoTask.atskdfSurveyvarNameIns')" v-model="atskdfSurveyvarName" @on-click="selectSurveyVar" />
							</span>
						</FormItem>
					</i-col>

					<i-col span="10" offset="1" v-if="$store.state.atsk.atskdfSmsvar">
						<FormItem :label="$t('lang_platform.autoTask.atskdfSmsvarName')" >
							<span @dblclick="dbSmsVar">
							<Input  readonly="readonly" :placeholder="$t('lang_platform.autoTask.atskdfSmsvarNameIns')" v-model="atskdfSmsvarName" @on-click="selectSmsVar" />
							</span>
						</FormItem>
					</i-col>
          <i-col span="10" offset="1" v-if="$store.state.atsk.atskdfWxvar">
            <FormItem :label="$t('lang_platform.autoTask.atskdfWxvarName')" >
							<span @dblclick="dbWxVar">
							<Input  readonly="readonly" :placeholder="$t('lang_platform.autoTask.atskdfWxvarNameIns')" v-model="atskdfWxvarName" @on-click="selectWxVar" />
							</span>
            </FormItem>
          </i-col>
					<i-col span="10" offset="1">
						<FormItem :label="$t('lang_platform.autoTask.atskdfGetvalbyclmName')" >
							...
							<Input  readonly="readonly" :placeholder="$t('lang_platform.autoTask.atskdfGetvalbyclmNameIns')" v-model="atskdfGetvalbyclmName" @on-click="pickData2"/>
							</span>
						</FormItem>
					</i-col>
			...
			<Row>
				<i-col span="21" offset="1">
					<Row type="flex" justify="end" class="btns">
						<Button type="" @click="close" style="margin-left: 8px">{{$t('button.cal')}}</Button>
...
//				logType: '',
        ...
        openPick2: false,
				...
				paramsSurVar: {
					_mt:  this.$global.mt+'PlatSurveyvar.getPage',
					sort: 'id',
					order: 'desc',
					rows: '10',
					page: 1,
      ...
      searchValbyfiled,
		...
		methods: {
			...
			getdata(rowId) {
				...
				t.spinShow = true;
				const params = {
					_mt:  this.$global.mt+'PlatAutotaskDatadefine.getById',
					id: rowId,
					funId: '1',
					logType: '查询List信息',
				}
      ...
      clearValbyfiled() {
        ...
        t.form.atskdfGetvalbyclm = '0'
			...
			save() {
				...
				params._mt =  this.$global.mt+'PlatAutotaskDatadefine.addOrUpd'
				params.funId = '1'
