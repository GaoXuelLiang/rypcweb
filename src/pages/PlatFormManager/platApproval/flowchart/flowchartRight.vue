  ...
  export default {
    ...
    methods: {
      ...
      getListByIdEdge(id) {
        const t = this
        const data = {
          _mt:  this.$global.mt+'PlatAprvsteprelation.getByFlowId',
          logType: this.$t('button.ser'),
          apstrelAprvid: id,
        }
        getDataLevelUserLoginNew(data).then((res) => {
      ...
      saveAll() {
        ...
        const data = {
          _mt:  this.$global.mt+'PlatAprvstep.updNodeInfo',
          logType: '保存',
      ...
      deleteLines() {
        ...
        let data2 = {
          ...
          APlogType: '刪除',
          APsourcenodeid: t.sourceNodeid,
          APtargetnodeid: t.targetNodeid,
        }
        getDataLevelUserLogin(data2).then((res2) => {
          if (isSuccess(res2, this)) {
            if (res2.data.content[0].value === 1) {
