	...
	<div class="cover">
		<div class="box">
			<div class="title">
              ...
              {{$t('lang_approval.dataBlock.fieldDeTitle')}}
		        </div>
			...
			<Row class="positionBox">
				<Col span="8">
					<!-- 可选择字段表格 -->
					<Table stripe :loading="loading" :no-data-text="noDataImg" ref="optionalField" :columns="optionalFieldColumns" :data="optionalFieldData" @on-selection-change="selectOpt" height="500"></Table>
      ...
      <span style="margin-left: 10px;margin-top: 2px"> {{$t('reminder.view')}} {{((page - 1) * rows + 1)}} - {{page*rows > total ? total : page*rows}}  {{$t('reminder.of')}}  <span>{{total}}</span></span><Button size="small" shape="circle" icon="md-refresh" style="margin-left: 20px;display: inline-block;" @click="getData(1)"></Button></Row> -->
				...
				<Col span="1">
					<ButtonGroup vertical size="small" class="btnFa">
						<Button size="small" icon="ios-arrow-up" @click="toTop(1)"></Button>
						<Button size="small" icon="ios-arrow-forward" @click="toRight()"></Button>
						<Button size="small" icon="ios-arrow-back" @click="toLeft()"></Button>
						<Button size="small" icon="ios-arrow-down" @click="toTop(2)"></Button>
					</ButtonGroup>
				</Col>
				<Col span="15">
					<!-- 已选择字段表格 -->
					<Table stripe :loading="loading" :no-data-text="noDataImg" ref="selectedField" :columns="selectedFieldColumns" :data="selectedFieldData" height="500" @on-selection-change="selectOpt2" @on-select-cancel="cancelSelect"></Table>
      ...
      <span style="margin-left: 10px;margin-top: 2px"> {{$t('reminder.view')}} {{((page - 1) * rows + 1)}} - {{page*rows > total ? total : page*rows}}  {{$t('reminder.of')}}  <span>{{total}}</span></span><Button size="small" shape="circle" icon="md-refresh" style="margin-left: 20px;display: inline-block;" @click="getData(1)"></Button></Row> -->
          <div style="text-align: right;">
            ...
            <!-- <Button type="primary" class="position" @click="saveField()" disabled>{{$t('button.sav')}}</Button> -->
          </div>
				</Col>
			</Row>
		</div>
...
<script>
  import {getDataLevelUserLoginData, getDataLevelUserLoginNew, getDataLevelUserLogin, getDataLevelUserLogin2 } from '../../../axios/axios'
  ...
  export default {
    ...
    methods: {
      ...
      getOptional(id) {
        const t = this
        const data = {
          _mt:  this.$global.mt+'PlatSformfield.getAppFieldList',
      ...
      getselected(id) {
        ...
        t.Datablock = id
        const data = {
          _mt:  this.$global.mt+'PlatAprvdbfield.getPage',
          aprvdbfdDatablock: t.Datablock,
          aprvalId: t.$store.state.platApproval.mainId,
            ...
            if (t.optSelected[dat1].sffFieldNameDis) {
              for (const dat2 in t.selectedFieldData) {
                if (t.selectedFieldData[dat2].aprvdbfdFieldDis) {
                  if (t.optSelected[dat1].sffFieldNameDis === t.selectedFieldData[dat2].aprvdbfdFieldDis) {
      ...
      saveField() {
        ...
        const data = {
          _mt:  this.$global.mt+'PlatAprvdbfield.addOrUpd',
          aprvdbfdDatablock: t.Datablock,
      ...
      moveLeft2(arr1, arr2) {
        for (const dat1 in arr1) {
          if (arr1[dat1].aprvdbfdFieldDis) {
            for (const dat2 in arr2) {
              if (arr2[dat2].sffFieldNameDis) {
                if (arr1[dat1].aprvdbfdFieldDis === arr2[dat2].sffFieldNameDis) {
                  arr2.splice(dat2, 1)
                }
              }
            }
...
.box{
  ...
  padding: 50px 20px 20px 20px; border-radius: 10px;
}
.title{
