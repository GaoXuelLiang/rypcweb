...
                  v-if="spinShow"></Spin>
      <div class="title">
        <div class="title-text">
          ...
          {{$t('lang_approval.wxvar.updTitle')}}
        </div>
      ...
      <Row class="content form_box">
        <Form ref="formValidate" disabled :model="formValidate" :rules="ruleValidate" label-position="right" :label-width="135">
          <i-col span="10" offset="1">
            <FormItem class="marNT" :label="$t('lang_approval.wxvar.aprvarMark')" prop="aprvarMark">
              <Input v-model="formValidate.aprvarMark" :placeholder="$t('lang_approval.wxvar.aprvarMarkDis')"></Input>
            </FormItem>
          </i-col>
          <i-col span="10" offset="1">
            <FormItem class="marNT" :label="$t('lang_approval.wxvar.aprvarName')" prop="aprvarName">
              <Input v-model="aprvarName" :disabled="true"></Input>
            </FormItem>
          </i-col>
        </Form>
  ...
  export default {
    data() {
      return {
        ...
        ruleValidate: {
          aprvarMark: [
            { required: true, message: this.$t('lang_approval.wxvar.aprvarMarkDis'), trigger: 'blur' },
          ],
        },
      }
    },
        ...
        this.$refs.formValidate.validate((valid) => {
          if (valid) {
            getDataLevelUserLoginNew(data).then((res) => {
              if (isSuccess(res, t)) {
                if (!data.id) {
                  t.$store.commit('platApproval/addNewArrayChildTable9', res.data.content[0])
                  t.$Modal.success({
                  ...
                  t.$store.commit('platApproval/updateArrayChildTable9', res.data.content[0])
                }
                t.close()
              }
            }).catch(() => {
