      ...
      <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="135">
        <Row>
          ...
          <FormItem :label="$t('lang_role.adminrole.roleCode')" prop="roleCode">
            <Input v-model="formValidate.roleCode" :placeholder="$t('lang_role.adminrole.proleCode')"
            ></Input>
          </FormItem>
          </Col>
          <Col span="10" offset="1">
          <FormItem :label="$t('lang_role.adminrole.roleCname')" prop="roleCname">
            <Input v-model="formValidate.roleCname" :placeholder="$t('lang_role.adminrole.pproleCname')"
            ></Input>
          </FormItem>
          </Col>
        </Row>
...
<!--            <Input v-model="formValidate.roleEname" :placeholder="$t('lang_role.adminrole.proleEname')"-->
<!--            ></Input>-->
<!--          </FormItem>-->
<!--          </Col>-->
          <Col span="10" offset="1">
          <FormItem :label="$t('lang_role.adminrole.validDis')" prop="valid">
            <RadioGroup v-model="formValidate.valid" type="button" size="small" :placeholder="$t('lang_role.adminrole.validDis')"
        ...
        <Row v-show="openInValid">
          ...
          <FormItem :label="$t('lang_role.adminrole.invdate')"  prop="invdate">
            <DatePicker type="date"  :placeholder="$t('lang_role.adminrole.pinvdate')"
                        :editable="false"
                        v-model="formValidate.invdate" style="width: 100%"></DatePicker>
          </FormItem>
          </Col>
          <Col span="10" offset="1">
  ...
  export default {
    data() {
      return {
        ...
        openInValid: false,
        formValidate: {
          _mt:  this.$global.mt+'SysESSMSSAgwService.addOrUpd',
          funId: '1',
          roleCode: '',
          roleCname: '',
          roleEname: '',
    ...
    methods: {
      getData(id) {
        ...
        getDataLevelUserLogin({
          _mt:  this.$global.mt+'AuthRole.getById',
          id: id,
        }).then((res) => {
          if (isSuccess(res, t)) {
