<template>
  <div>
    <Row>
      <Col class="col" span="6"  id="col3" style="margin-right: 10px">
      ...
      <Col span="17" id="col4">
        ...
        <Tabs type="card" name="name2">
        <TabPane :label="$t('lang_role.adminrole.flagBox')" style="height:450px;"
                 :disabled="flagBox.funIsctrlbtn !== '1'" tab="name2">
          <div v-if="flagBox.funIsctrlbtn === '1'">
            <div class="btn-group">
              <Button type="primary" @click="selectAllCheck">{{$t('lang_user.rootrole.allPick')}}</Button>
              <Button type="primary" @click="checkReC">{{$t('lang_user.rootrole.checkReC')}}</Button>
  ...
  export default {
    data() {
      return {
        ...
        data4: [],
        params: {
          _mt:  this.$global.mt+'SysESSMSSAgwService.selPartSysFunctionsByTree',
          funType: '1admin',
        },
    ...
    methods: {
          ...
          if (isSuccess(res, t)) {
            ...
            this.$nextTick(() => {
              if (document.body.clientHeight <= 630) {
                ...
                document.getElementById('col3').style.height = '410px'
                document.getElementById('col4').style.height = '410px'
              }
            })
          }
        }).catch(() => {
      ...
      * */
      getData() {
//        console.log(document.bodyF.clientHeight)
        const t = this
        ...
        getDataLevelUserLoginNew(data).then((res) => {
          t.isSpin1 = false
          if (isSuccess(res, t)) {
            if (res.data.content[0] && res.data.content[0].value) {
      ...
      * */
      getCheckBox(id) {
        const t = this
        const data = {
          _mt:  this.$global.mt+'AuthRolePower.getFunBtn',
          btnFunId: id,
          roleid: t.id,
          logType: this.$t('button.ser'),
        }
      ...
      * */
      saveBtn() {
        ...
        getDataLevelUserLogin({
          _mt:  this.$global.mt+'AuthRolePower.addFunBtn',
          btnLancodes: codes,
          btnFunId: t.btnFunId,
          roleid: t.id,
          logType: '保存',
