<template>
  <div class="table">
    <Row>
      ...
      <card>
        <p slot="title">
          ...
          {{$t('lang_role.essrole.title3')}}
        </p>
        <Row class="btnHeight">
          <Input class="searchInput" :placeholder="$t('lang_user.rootuser.placeholdername')" style="width: 200px" v-model="userName"/>
        ...
        <row class="table-form" ref="table-form">
          <Table stripe :loading="loading" :height="tableheight" :no-data-text="noDataImg" @on-select="selectedtable" @on-select-cancel="selectedtable" @on-select-all="selectedtable" @on-select-all-cancel="selectedtable"
                 @on-sort-change="sortable" size="small" ref="selection" :columns="columns"
                 :data="data"></Table>
        </row>
    ...
    <transition name="fade">
      <updStateUser v-show="updState" :roleGrpName="roleGrpName" :searchText="searchText" :searchCloumns="searchCloumns"
                    @close="close" @changeinput="changeinput" :sentParams='params' ref="updStateUser"></updStateUser>
    </transition>
  </div>
</template>
<script>
...
import flowStepData from '../../PlatFormManager/platFlowStep/flowStepData'
...
//          width: 150,
          ...
          {
            title: this.$t('lang_user.rootuser.loginName'),
            key: 'loginName',
            sortable: 'custom',
//          width: 150,
          },
