    ...
    <Row>
      ...
      <card>
        ...
        <Row class="btnHeight">
          ...
          <Button style="margin-left: 5px;" type="warning" @click="openUp('',$t('button.add'))" v-show="sysAdminStatus !=='03invalid' ? true : false">{{$t('button.add')}}
          </Button>
          <Button style="margin-left: 5px;" type="error" @click="deletemsg('0')" v-show="sysAdminStatus ==='02valid' ? true : false">{{$t('lang_user.rootuser.invalid')}}</Button>
          <span style="margin-left: 5px;" v-show="sysAdminStatus ==='03invalid' ? true : false">
          <Button type="primary" @click="deletemsg('1')">{{$t('lang_user.rootuser.valid')}}</Button>
             </span>
        </Row>
        <!--<Row>-->
        ...
        <row class="table-form" ref="table-form">
          <Table @on-select="selectedtable" @on-select-cancel="selectedtable" :loading='loading' @on-select-all="selectedtable" @on-select-all-cancel="selectedtable"
                 @on-sort-change="sortable"  size="small" stripe ref="selection" :columns="columns"
                 :data="data" :height='tableheight' :no-data-text="noDataImg"></Table>
        </row>
            ...
            render: (h, params) => {
              return h('div', [
                h('Button', {
                  ...
                  style: {
                    marginRight: this.sysAdminStatus === '02valid' ? '10px' : '0px',
          ...
          t.$Modal.confirm({
            ...
            onOk: () => {
              getDataLevelUserLogin({
                _mt:  this.$global.mt+'SysUserinfo.delByIds',
