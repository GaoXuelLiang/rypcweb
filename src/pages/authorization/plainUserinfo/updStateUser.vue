<template>
  <div class="background">
    <div class="backbox">
      ...
      <!-- 选项卡 开始-->
      <Tabs value="name1" @on-click="changeTab" ref="tab" type="card">
        <TabPane :label="$t('lang_role.adminuser.assignrole')"  name="name1">
          <Row class="table-form">
            ...
            <span style="margin: 0;"><Button type="info" style="width:56px" :loading="isSpins" @click="getData(sentParams)"><span v-if="!isSpins">{{$t('button.ser')}}</span></Button></span>
      ...
      <span style="margin-left: 10px;margin-top: 2px"> {{$t('reminder.view')}} {{((page - 1) * rows + 1)}} - {{page*rows > total ? total : page*rows}}  {{$t('reminder.of')}}  <span>{{total}}</span></span><Button size="small" shape="circle" icon="md-refresh" style="margin-left: 20px;display: inline-block;" @click="getData(1)"></Button></Row>-->
        ...
        <TabPane :label="$t('lang_role.adminuser.assignorg')"  name="name2">
          <Row>
            <Col class="col border_2" span="19" style="position: relative;">
...
export default{
  ...
  methods: {
    ...
    // 分配角色
      getData(sentParams, id) {
        const t = this;
        t.loading = true;
        t.isSpins = true;
        const data = deepCopy(sentParams)
        t.dataObj = deepCopy(sentParams)
        t.id = id || t.id
    ...
    dataProcessing() {
      ...
      if (undefined === t.selectData || t.selectData.length === 0) {
        ...
        for (let i = 0; i < t.wholeDdata.length; i++) {
          let map = {}
          map['compSnameCnDis'] = t.wholeDdata[i].compSnameCnDis
          map['expand'] = false
