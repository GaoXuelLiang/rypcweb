    ...
    <div class="backbox">
      <div class="title">
        <div class="title-text">
          <Icon type="ios-bookmark" size="16"></Icon>&nbsp;{{$t('lang_evaluation.evaluation.searchScheme')}}
        </div>
        <Button type="text"  @click="closeScheme">
  ...
  export default{
    ...
    methods: {
      ...
      getData() {
        const t = this
        const data = {
	      _mt:  this.$global.mt+'EvaluationPro.getPage',
	      rows: t.rows,
	      page: t.page,
	      sort: t.sort,
	      order: t.order,
      ...
      dbCkick(row) {
        const t = this
        this.$emit('changeinputScheme', row.id, row.proName)
        t.closeScheme()
      },
      sizeChange(size) {
        const t = this
