<template>
    <div class="table">
                ...
                <Row>
                    ...
                    <Row>
                        <span @dblclick="clearUserid">
                            <Input :placeholder="$t('lang_evaluation.evaluation.resYear2Ins')"
                                   style="width: 200px"
                                   v-model="resEmpTimeName" />
                        </span>
                        <span @dblclick="clearUserid">
                            <Input :placeholder="$t('lang_evaluation.evaluation.resEmpidDis1Ins2')"
                                   icon="ios-search"
                                   :readonly="true"
                                   style="width: 200px"
                                   v-model="resEmpidDis"
                        ...
                        <span @dblclick="clearUserid">
                            <Input :placeholder="$t('lang_evaluation.evaluation.resEmpSchemeNameIns2')"
                                   icon="ios-search"
                                   :readonly="true"
                                   style="width: 200px"
                                   v-model="resEmpSchemeName"
                                   @on-click="pickUserSchemeData" />
                        </span>
        ...
        <transition name="fade">
            <diagnosticScoreResult v-show="openScoreResult"
                                   @closePower="closeScoreResult"
                                   ref="diagnosticScoreResult"></diagnosticScoreResult>
        </transition>
        <!-- 诊断方案 -->
        <transition name="fade">
            <searchScheme v-show="openScheme"
                          @closeScheme="closeScheme"
                          @changeinputScheme="changeinputScheme"
                          ref="searchScheme"></searchScheme>
        </transition>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        getData () {
            const t = this
            const data = {
                _mt:  this.$global.mt+'EvaluationResult.getPage',
                rows: t.rows,
                page: t.page,
                sort: t.sort,
                order: t.order,
                ...
                t.$Modal.confirm({
                    ...
                    onOk: () => {
                        getDataLevelUserLogin({
                            _mt:  this.$global.mt+'EvaluationResult.delByIds',
                            funId: '1',
