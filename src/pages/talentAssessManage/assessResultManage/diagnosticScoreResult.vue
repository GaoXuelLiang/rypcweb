    ...
    <div class="box">
      <div class="title">
        <div class="title-text">
          <Icon type="ios-bookmark" size="16"></Icon>&nbsp;{{$t('lang_evaluation.evaluation.diagnosticScoreResult2')}}
        </div>
      ...
      <Row>
        <Col class="col" span="18">
            ...
            <span style="height:25px;display:inline-block;text-align:center">{{$t('lang_evaluation.evaluation.diagproject2')}}</span>
            ...
            <span style="height:25px;display:inline-block;float:right;width:100px;margin-right:20px;text-align:center">{{$t('lang_evaluation.evaluation.diagscoring2')}}</span>
          </div>
  ...
  export default {
    ...
    methods: {
      getData(id) {
        ...
        t.id = id;
        const data={
        	_mt:  this.$global.mt+'EvaluationProinfo.getListByResid',
...
//        })
      ...
      save() {
        ...
        const data = {
          _mt:  this.$global.mt+'EvaluationProinfo.addOrUpd',
          logType: '指标评分结果保存',
          data: this.score,
        }
