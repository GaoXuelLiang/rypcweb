    ...
    <Row>
      ...
      <card>
        <p slot="title">
          ...
          &nbsp;{{$t('lang_evaluation.evaluation.assessNomal')}}
        </p>
        <Row>
...
<script>
  import update from './upAssessNomalManage'
  ...
  export default{
    data() {
      return {
        ...
        openImport: false,
        expDataTital: [{ code: 'indName', name: '指标名称' }, { code: 'indPidDis', name: '上级指标' },
          { code: 'indOrder', name: '显示序号'}, { code: 'indModeDis', name: '获取结果方式' },
          { code: 'indIden', name: '系统标识' }, { code: 'indHighestscore', name: '最高分' },
          { code: 'indExplain', name: '指标说明' }],
        indName: '',
        indPid: '',
        openExpDow: false,
        openExp: false,
    ...
    methods: {
      ...
      getTree() {
        const t = this
        const data = {
          _mt:  this.$global.mt+'EvaluationIndex.getTree',
          indType: '03eval',
          logType: this.$t('button.ser'),
        }
        for (const dat in data) {
          ...
          t.$Modal.confirm({
            ...
            onOk: () => {
              getDataLevelUserLogin({
                _mt:  this.$global.mt+'EvaluationIndex.delByIds',
                funId: '1',
