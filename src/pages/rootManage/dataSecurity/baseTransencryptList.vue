		...
		<Row>
			...
			<card>
				...
				<Row>
					<Col span="24">
						<Row>
							<Input v-model="searchParams.baseTrseptNo" placeholder="请输入序号" style="width: 150px"/>
							<Input v-model="searchParams.baseTrseptProperty" placeholder="请输入字段属性" style="width: 150px"/>
							<!-- 页面按钮 -->
							...
							<Button type="primary" v-if="searchParams.baseTrseptStatus === '01draft' || searchParams.baseTrseptStatus === '' " @click="openUp(NaN,$t('button.add'))">{{$t('button.add')}}</Button>
							<Button type="success" v-if="searchParams.baseTrseptStatus === '01draft'" @click="effectData('02valid')">生效</Button>
							<Button type="error" v-if="searchParams.baseTrseptStatus === '01draft'" @click="deletemsg">删除</Button>
						</Row>
						<!-- 表格 分页 -->
...
import update from "./baseTransencryptEdit";                    //新增修改组件
...
export default {
	...
	data () {
		return {
			labelDesc: {baseTrseptNo:"",baseTrseptProperty:"",baseTrseptDesc:"",baseTrseptStatusDis:"",baseTrseptVdate:"",comment:"",},
			placeHolderDesc: {baseTrseptNo:"",baseTrseptProperty:"",baseTrseptDesc:"",baseTrseptStatusDis:"",baseTrseptVdate:"",comment:"",},
			ruleDesc: {baseTrseptNo:"",baseTrseptProperty:"",baseTrseptDesc:"",baseTrseptStatusDis:"",baseTrseptVdate:"",comment:"",},
			// 导入的mt名称
			imp_mt:  this.$global.mt+'BaseTransencrypt.importData',
			// 导出的mt名称
			exp_mt:  this.$global.mt+'BaseTransencrypt.export',
			// 导出字段设置, code字段名 name列名
			expDataTital: [
				{ code: "baseTrseptNo", name: "" },
				{ code: "baseTrseptProperty", name: "" },
				{ code: "baseTrseptDesc", name: "" },
				{ code: "baseTrseptStatusDis", name: "" },
				{ code: "baseTrseptVdate", name: "" },
				{ code: "comment", name: "" },
			],
			// 表格列字段
			columns: [
				{ type : "selection" , width: 54 , fixed : "left" , align : "center" },
				{ key: "baseTrseptNo", title: "", sortable: "custom" , width : 220},
				{ key: "baseTrseptProperty", title: "", sortable: "custom" , width : 220},
				{ key: "baseTrseptDesc", title: "", sortable: "custom" , width : 220},
				{ key: "baseTrseptStatusDis", title: "", sortable: "custom" , width : 220},
				{ key: "baseTrseptVdate", title: "", sortable: "custom" , width : 220},
			],
			// 表格获取数据mt名称
			page_mt:  this.$global.mt+'BaseTransencrypt.getPage',
			// 删除数据mt名称
			dele_mt:  this.$global.mt+'BaseTransencrypt.delByIds',
			// 修改流程状态mt名称
			state_mt:  this.$global.mt+'BaseTransencrypt.setStateByIds',
			//表格 id
			updateId: NaN,
			//操作类型
			logType: "",
    ...
    beforeCreate () {
		...
		created() {
			const t = this
     	let tempObj = {}
			tempObj.baseTrseptNo = this.searchParams.baseTrseptNo
			tempObj.baseTrseptProperty = this.searchParams.baseTrseptProperty
			tempObj.baseTrseptStatus = this.searchParams.baseTrseptStatus
      ...
      if (eval(getUrlKey('state').toLowerCase())) {
				const t = this
				let tempObj = {}
				this.searchParams.baseTrseptStatus = '02valid'
				this.searchParams.baseTrseptProperty = ''
				this.searchParams.baseTrseptNo = ''
				this.selectDis = {
					ebpStatusDis: '已生效',
					ebpTypeDis: '',
				}
    ...
    methods: {
        ...
        getColumns () {
	        const t = this
	        let lanCodes = 'base_trsept_no,base_trsept_property,base_trsept_desc,base_trsept_status,baseTrseptStatusDis,base_trsept_vdate,comment'
        ...
        selectedEbstatus(key, value) {
          this.searchParams.baseTrseptStatus = key
        ...
        effectData(status) {
          ...
          getDataLevelUserLogin({
            _mt:  this.$global.mt+'BaseTransencrypt.updState',
