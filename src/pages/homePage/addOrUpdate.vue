        ...
        <Form :model="form"
            ...
            <FormItem label="提醒范围">
                <Select v-model="form.remindType"
                        style="width:150px;">
                    <Option v-for="(item,index) in remindTypeList"
                            :key="index"
            ...
            <FormItem label="提醒">
                <Row>
                    <Col span="2"
                         v-if="form.remindAdvance !== 'nothtype'">
                    提前
                    </Col>
                    <Col span="5"
                         v-if="form.remindAdvance !== 'nothtype'">
                    <InputNumber :max="10"
                                 :min="1"
                                 v-model="form.remindAdvanceNum"></InputNumber>
                    </Col>
                    <Col span="5"
                         offset="1">
                    <Select v-model="form.remindAdvance"
                            @on-change="selectChange">
                        <Option v-for="(item,index) in timetype"
                                :key="index"
                                :value="item.paramCode">{{item.paramInfoCn}}</Option>
                    </Select>
                    </Col>
            ...
            <FormItem label="添加成员">
                <!-- <Row> -->
                <!-- <Col span="2"> -->
                <Tag v-for="(item, index) in empArr"
                     closable
            ...
            <FormItem label="描述和附件">
                <Input v-model="form.remindFiledesc"
                ...
                <label class="filelabel"
                       for="fileinputModel">上传附件</label>
                文件名称 : &nbsp;
                <a href="javascript:;"
                   @click="download(file.url)">{{ file.name }}</a>
            </FormItem>
            <FormItem label="是否重复">
            ...
            <FormItem label=""
                      v-show="form.remindIsrep === '1'">
                <a href="javascript:;"
                   @click="newTodo">重复</a>
            </FormItem>
            <div class="btn"
                 style="text-align: right;padding-bottom: 10px">
                <Button @click="closeMain">取消</Button>
                <span v-show="dateShow">
                    <Button type="error"
                            v-if="id"
                            @click="isDeleteAll">删除</Button>
                    <Button type="primary"
                            @click="addOrdate">确定</Button>
                </span>
            </div>
        </Form>
        <el-dialog title="重复"
                   width='850px'
                   :visible.sync="updateOrAdd"
                   @close="reset2"
                   append-to-body>
            <Form :model="repeat"
                  ref="form2"
                  ...
                  :label-width="100">
                <FormItem label="重复类型">
                    <Select v-model="repeat.remindRepeattype"
                            style="width:150px;"
                ...
                <FormItem label="重复频率">
                    <Row>
                        ...
                        <InputNumber :max="10"
                                     :min="1"
                                     v-model="repeat.remindRepeatnum"></InputNumber>
                        </Col>
                        <Col span="1">
                        {{repeatTypep}}
                        </Col>
                ...
                <FormItem label="星期"
                          prop="remindRepeatweek"
                          v-if="repeat.remindRepeattype === 'repeatweekly'">
                    <CheckboxGroup v-model="repeat.remindRepeatweek"
                ...
                <FormItem label="结束">
                    <Col span="5">
                    <Select v-model="repeat.remindRepeatstoptype"
                            @on-change="selectChange2">
                        <Option v-for="(item,index) in enddate"
                                :key="index"
                                :value="item.paramCode">{{item.paramInfoCn}}</Option>
                    </Select>
                    </Col>
                    <Col span="5"
                         offset="1"
                         v-if="repeat.remindRepeatstoptype === 'enddatefrequency'">
                    <InputNumber :max="10"
                                 :min="1"
                                 v-model="repeat.remindRepeatcount"></InputNumber>
                    次
                    </Col>
                    <Col span="7"
                         offset="1"
                         v-if="repeat.remindRepeatstoptype === 'enddatedate'">
                    <FormItem prop="remindRepeatstop">
                        <DatePicker type="date"
                                    placeholder="选择结束日期"
                                    v-model="repeat.remindRepeatstop"
          ...
          {{repeat.res}}
                ...
                <div class="btn"
                    ...
                    <Button type="primary"
                            @click="closeNew">确定</Button>
                </div>
            </Form>
        </el-dialog>
        <div style="position: fixed;top: 0;left: 0;bottom: 0;right: 0;background: rgba(0,0,0,0.8);z-index: 3000"
             v-if="isAll">
            <div style="background: white;width:260px;position: absolute;margin-left: 500px;margin-top: 158px;padding: 10px">
                ...
                <div class="btn"
                     style="text-align: right">
                    <Button type="primary"
                            @click="changeOne">当前任务</Button>
                    <Button type="primary"
                            @click="changeAll">全部任务</Button>
                </div>
            </div>
        </div>
        <div style="position: fixed;top: 0;left: 0;bottom: 0;right: 0;background: rgba(0,0,0,0.8);z-index: 3000"
            ...
            <div style="background: white;width:260px;position: absolute;margin-left: 500px;margin-top: 158px;padding: 10px">
                ...
                <div class="btn"
                     style="text-align: right">
                    <Button type="primary"
                            @click="deleteEvent(2)">当前任务</Button>
                    <Button type="primary"
                            @click="deleteEvent(3)">全部任务</Button>
                </div>
            </div>
        </div>
        <searchPubEmp @closeUp="closeUp"
                      v-if="showEmp"
    ...
    methods: {
        ...
        getSelect () {
            ...
            getDataLevelUserLoginData({
                ...
                APtypeCode: 'enddate,repeattype,timetype,remindtype',
            }).then((res) => {
                if (isSuccess(res, t)) {
