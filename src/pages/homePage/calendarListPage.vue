<template>
    <div class="background">
        <div class="backbox">
            ...
            <div class="content">
                <div class="title">
                    <div class="left">
                        <Button size="small"
                                @click="backTody">今天</Button>
                        <Button size="small"
                                icon="ios-arrow-back"
                                @click="lastMonth"></Button>
                        <Button size="small"
                                icon="chevron-right"
                        ...
                        <DatePicker :open="open"
                                    ...
                                    @on-ok="handleOk">
                            <div class="date_text"
                                 @click="handleClick">{{year +'年'+ month + '月'}}</div>
                        </DatePicker>
                    </div>
                    <div class="right">
                ...
                <div class="calendar">
                    <div class="cal_title">
                        <strong v-for="(item, index) in fullWeekMap"
                                :key="index">{{item}}</strong>
                    </div>
                    <div class="cal_body"
                         v-for="(num, i) in dayList"
                         :key="i">
                        <div class="day_cell"
                             :class="[ item.type, { 'now_date': item.isToday, 'selected': item.selected }]"
                             v-for="(item, index) in num"
                             :key="index"
                             @click="cellClick(i, index)">
                            <div class="num">{{item.date}}</div>
                            <div class="events"
                                 v-for="(item2, index2) in item.events"
                                 :key="index2"
                                 @click="changeEventP(item2.id)">{{item2.title}}</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div style="position: fixed;top: 0;left: 0;bottom: 0;right: 0;background: rgba(0,0,0,0.8);z-index: 3000"
             v-if="changeEvent">
            <div style="background: white;width:850px;position: absolute;margin-left: 300px;margin-top: 50px;">
                ...
                <addOrUpdate :dateShow="dateShow"
                             ...
                             :id="id"
                             :state="state"
                             @closechang="closechang"
                             @rerender="rerender"></addOrUpdate>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        getData (startTime, endTime) {
            let t = this
            getDataLevelUserLoginData({
                _mt: this.$global.mt + 'CmutRemind.getdataByList',
        ...
        close1 () {
            this.changeEvent = false
            this.$refs.addOrUpdate.typeShow = true
        },
    },
}
