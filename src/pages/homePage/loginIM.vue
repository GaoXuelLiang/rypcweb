    ...
    <div class="cover">
        <div class="box">
            ...
            <Form ref="formValidate"
                ...
                <Row>
                    <FormItem label="用户名">
                        <Input v-model="formValidate.userIM"
                               placeholder="请输入用户名...">
                        </Input>
                    </FormItem>
                </Row>
                <Row>
                    <FormItem label="密码">
                        <Input v-model="formValidate.passwordIM"
                               placeholder="请输入密码...">
                        </Input>
                    </FormItem>
                </Row>
    ...
    methods: {
        ...
        getIdMessage (id) {
            ...
            getDataLevelUserLoginData({
                ...
                if (isSuccess(res, t)) {
                    t.userName = res.data.content[0].name
                    t.userImage = pubsource.pub_pubf_downlink + res.data.content[0].pictureShrink.split(',')[1]
        ...
        loginIM () {
            ...
            getDataLevelUserLogin({
                _mt: this.$global.mt + 'UserInfo.getuserbyid',
                APid: t.$store.state.user.userId,
            }).then((res) => {
                if (isSuccess(res, t)) {
                    ...
                    t.initData.mine = {
                        ...
                        id: res.data.content[0].id + '_' + pubsource.webSocketDevice,
                    ...
                    // *******获取好友和群组的信息***********
                    getDataLevelUserLogin({
                        _mt: this.$global.mt + 'CmutSupportteam.getTeamAndMembers',
                        sort: '',
                        order: '',
                    }).then((res) => {
                        if (isSuccess(res, t)) {
                            ...
                            if (res.data.content[0].value) {
                                ...
                                for (let i = 0; i < res.data.content[0].value.length; i++) {
                                    ...
                                    for (let j = 0; j < res.data.content[0].value[i].memList.length; j++) {
                                        ...
                                        t.initData.friend[i + 1].list.push({
                                            ...
                                            id: res.data.content[0].value[i].memList[j].cmstmUserid + '_' + pubsource.webSocketDevice,
                                ...
                                console.log(this.initData, '111111111111111111111')
                                // 信息获取完成再打开主面板
        ...
        initIM () {
            ...
            layui.use('layim', function (layim) {
                ...
                let initEventHandle = function () {
                    ...
                    socket.onmessage = function (event) {
                        ...
                        if (event.data instanceof ArrayBuffer) {
                            ...
                            //                console.log(msg.getCmd(), '这里是收到的消息1---getCmd')
                            let msgCon = proto.MessageBody.deserializeBinary(msg.getContent());
                            console.log(msgCon, '这里是收到的消息2')
                            //                console.log(msgCon.getContent(), '这里是收到的消息2-msgCon.getContent()')
                            //                console.log(msg.getSender(), '这里是收到的消息2-msg.getSender()')
                            let cache = layui.layim.cache();
                            //                t.getIdMessage(msg.getSender().split('_')[0])
                            ...
                            if (msg.getCmd() == 2) {
                                ...
                                //显示非自身消息
                                //                  console.log(msg.getGroupid(), '11111111')
                                //                  console.log(username, '11111111')
                                //                  console.log(msg.getGroupid(), '11111111')
                                if (msg.getSender().split('_')[0] !== '0') {
                                    t.getIdMessage(msg.getSender().split('_')[0])
                                }
                                if (msg.getSender().split('_')[0] === '0') {
                                    t.userName = 'HR智能助手'
                                    t.userImage = '../../../static/workOrderImg/6.png'
                                }
                                setTimeout(() => {
                                    if (msg.getSender() != t.currentsession) {
                                        //不显示用户组消息
                                        ...
                                        if (msg.getGroupid() == null || msg.getGroupid().length < 1) {
                                            layim.getMessage({
                                                username: t.userName
                                                , avatar: t.userImage
                                                // , avatar: t.userImage + "?" + new Date().getTime()
                                                , id: msg.getSender()
                                                , type: "friend"
                ...
                //          console.log(layui.layim.cache(), '44444444444444444444444')
