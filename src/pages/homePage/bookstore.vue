<template>
    <i-col class="col"
        ...
        <card :bordered="false">
            <p slot="title"
               class="title">
                <Icon type="ios-book"></Icon> <span style="margin-left: 5px;">{{$t('lang_homePage.bookStore.title')}}</span>
            </p>
            <a href="#"
               slot="extra"
               @click="lookStore">
        ...
        <transition name="fade">
            <bookdetail v-show="isDetail"
                        ref="bookdetail"
                        @closeUp="closeUp"></bookdetail>
        </transition>
    </i-col>
</template>
...
export default {
    ...
    methods: {
        ...
        getData () {
            const t = this
            const data = {
                _mt: this.$global.mt + 'CmutKnowledge.getList',
                APfunId: 1,
                APsort: 'cmutKldPublish',
                APorder: 'desc',
                APcmutKldState: '03released',
            }
