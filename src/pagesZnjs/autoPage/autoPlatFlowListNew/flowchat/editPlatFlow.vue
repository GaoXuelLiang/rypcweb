        ...
        <div class="box">
            ...
            <Form ref="formValidate"
                ...
                <Row>
                    <Col span="10"
                         offset="1">
                    <FormItem :label="$t('lang_flow.flow.flowCode')"
                              prop="flowCode">
                        <Input v-model="formValidate.flowCode"
                               :disabled="editDisbale"
                               :placeholder="$t('lang_flow.flow.p_flowCode')"></Input>
                    </FormItem>
                    </Col>
                    <Col span="10"
                         offset="1">
                    <FormItem :label="$t('lang_flow.flow.flowName')"
                              prop="flowName">
                        <Input v-model="formValidate.flowName"
                               :disabled="editDisbale"
                               :placeholder="$t('lang_flow.flow.p_flowName')"></Input>
                    </FormItem>
                    </Col>
                </Row>
                <Row>
                    <Col span="10"
                         offset="1">
                    <FormItem :label="$t('lang_flow.flow.flowFormtype')"
                              prop="flowFormtype"
                              :placeholder="$t('lang_flow.flow.p_flowFormtype')"
                              transfer>
                        <Select v-model="formValidate.flowFormtype"
                                :disabled="editDisbale">
                    ...
                    <Col span="10"
                         offset="1">
                    <FormItem :label="$t('lang_flow.flow.flowForm')"
                              prop="flowForm">
                        <span @dblclick="clear">
                            <Input v-model="flowFormDis"
                                   :disabled="editDisbale"
                                   icon="ios-search"
                                   :readonly="true"
                                   :placeholder="$t('lang_flow.flow.p_flowForm')"
                                   @on-click="pickData" />
                        </span>
                    </FormItem>
                    </Col>
                ...
                <Row>
                    <Col span="10"
                         offset="1">
                    <FormItem :label="$t('lang_flow.flow.flowVersion')"
                              prop="flowVersion">
                        <Input v-model="formValidate.flowVersion"
                               :disabled="editDisbale"
                               :placeholder="$t('lang_flow.flow.p_flowVersion')"></Input>
                    </FormItem>
                    </Col>
                    <Col span="10"
                         offset="1">
                    <FormItem :label="$t('lang_flow.flow.flowStatus')"
                              prop="flowStatus">
                        <Select v-model="formValidate.flowStatus"
                                disabled
                                :placeholder="$t('lang_flow.flow.p_flowStatus')"
                                transfer>
                            <Option :value="item.paramCode"
                                    v-for="(item,index) in selectFlowStatus"
                                    :key="index">{{item.paramInfoCn}}</Option>
                        </Select>
                    </FormItem>
                    </Col>
                ...
                <Row>
                    <Col span="21"
                         offset="1">
                    <FormItem :label="$t('lang_flow.flow.flowAfthdClass')"
                              prop="flowAfthdClass">
                        <Input v-model="formValidate.flowAfthdClass"
                               :disabled="editDisbale"
                               :placeholder="$t('lang_flow.flow.p_flowAfthdClass')"></Input>
                    </FormItem>
                    </Col>
                </Row>
                <Row>
                    <Col span="21"
                         offset="1">
                    <FormItem :label="$t('lang_flow.flow.comment')"
                              prop="comment">
                        <Input v-model="formValidate.comment"
                               :disabled="editDisbale"
                               type="textarea"
                               :autosize="{minRows: 2,maxRows: 5}"
                               :placeholder="$t('lang_flow.flow.p_comment')"></Input>
                    </FormItem>
                    </Col>
                </Row>
                <Row>
...
export default {
    data () {
        ...
        return {
            ...
            flowFormDis: '',
            formValidate: {
                _mt:  this.$global.mt+'PlatFlow.addOrUpd',
                flowCode: '',
                flowName: '',
                flowStatus: '01draft',
                flowFormtype: '',
    ...
    methods: {
        getData (id) {
            ...
            getDataLevelUserLogin({
                _mt:  this.$global.mt+'PlatFlow.getById',
                id: id,
                funId: '1',
                logType: '反查',
            }).then((res) => {
        ...
        handleReset () {
            this.$emit('closeUp')
            this.formValidate.flowStatus = '01draft'
        ...
        clear () {
            ...
            t.formValidate.flowCode = ''
            t.formValidate.flowName = ''
            t.formValidate.flowStatus = ''
            t.flowStatus = ''
            t.formValidate.flowFormtype = ''
            t.flowFormtypeDis = ''
            t.flowFormDis = ''
            t.formValidate.flowForm = ''
            t.formValidate.flowVersion = ''
            t.formValidate.flowAfthdClass = ''
