              ...
              <div style="margin-top: 20px;">
                ...
                <div class="btnStatus" v-show="btnStatus" v-for="(item,index) in checkbox" :key="index">
                  ...
                  <div class="btnList" v-show="item.bNoCheckSign=='1'">
                    <!-- <CheckboxGroup v-model="btnsCheck[item.bNoCode]" @on-change="check"> -->
                      <Checkbox v-for="(x,y) in item.buttonList" 
                                :key="y" 
                                :id="x.btnLancode"
                                :value="x.btnCheckSign=='1'?x.btnLancode:'0'"
                                :true-value="x.btnLancode"
                                false-value="0"
                                @on-change="check">
    ...
    methods: {
      ...
      * */
      ...
      selectChange(param) {
        ...
        if (param[0].funProcesid && param[0].funProcesid!=='0') {
        ...
        if(this.$global.mt=='znjs'||this.$global.mt=='xian'){
          // this.$nextTick(()=>{
          //   let dom = document.getElementsByClassName('ivu-tree-title-selected')
          //    dom[0].style.color="red";

          //   console.log(dom,'888888888')
          // })
          this.getBtnRole(param[0].id)
        }
      },
      getWay(value) {
        this.stepWay = value
      ...
      getBtnRole(id){
        const t = this
         getDataLevelUserLoginData({
            _mt:  this.$global.mt+'AuthRolePower.getRoleFun',
            AProleId: t.id,
            APfunId:id
         }).then((res) => {
          if (isSuccess(res, t)) {
            t.stepWay = res.data.content[0].flowstepdis
          }
        }).catch(()=>{
           t.$Modal.error({
            title: this.$t('reminder.err'),
...
//        t.copyArr[t.faIndex].dbList[t.chIndex].handleStatus = t.chValue
      ...
      * */
      ...
      check(value){
        ...
        let preNodeid = event.target.parentNode.parentNode.parentNode.previousElementSibling.id;
        let checked = event.target.checked;
        if(checked) {
          this.btnsCheck[preNodeid].push(nodeid)
        }else {
          let index = this.btnsCheck[preNodeid].indexOf(nodeid)
          this.btnsCheck[preNodeid].splice(index, 1)
        }
      },
      statusSelect(value){
        const t = this;
      ...
      * */
      ...
      saveColumn4() {
        ...
        getDataLevelUserLoginData(data).then((res) => {
          console.log(data,'shuzjsadaaaaaa')
          if (isSuccess(res, t)) {
            this.$Modal.success({
              title: this.$t('reminder.suc'),
              content: this.$t('reminder.savsuccess'),
