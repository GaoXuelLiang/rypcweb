  ...
  <div class="background">
    <div class="backbox">
      ...
      <Row class="table-form">
            ...
            <span><Button class="btnStyle" style="width:56px" type="info" :loading="isSpin" @click="getData(id)"><span v-if="!isSpin">{{$t('button.ser')}}</span></Button></span>
            <span><Button class="btnStyle" type="warning" @click="clear(1)">{{$t('button.cle')}}</Button></span>
          </Row>
      ...
      <span style="margin-left: 10px;margin-top: 2px"> {{$t('reminder.view')}} {{((costParams.APpage - 1) * costParams.AProws + 1)}} - {{costParams.APpage*costParams.AProws > total ? total : costParams.page*costParams.rows}}  {{$t('reminder.of')}}  <span>{{total}}</span></span><Button size="small" shape="circle" icon="md-refresh" style="margin-left: 20px;display: inline-block;" @click="getData(1)"></Button></Row>
      ...
      <!--<Tabs value="name1">
        <TabPane label="分配角色" name="name1">
          <Row class="table-form">
            ...
            <span style="margin: 0;"><Button type="primary"  @click="getData(sentParams)">{{$t('button.ser')}}</Button></span>
            <span style="margin: 0;"><Button type="warning"   @click="clear(1)">清空</Button></span>
          </Row>
          <row class="table-form" ref="table-form">
            <i-col span="19">
      ...
      <span style="margin-left: 10px;margin-top: 2px"> {{$t('reminder.view')}} {{((page - 1) * rows + 1)}} - {{page*rows > total ? total : page*rows}}  {{$t('reminder.of')}}  <span>{{total}}</span></span><Button size="small" shape="circle" icon="md-refresh" style="margin-left: 20px;display: inline-block;" @click="getData(1)"></Button></Row>
        ...
        <TabPane label="分配成本中心" name="name3">
          <Row class="table-form">
            ...
            <Button type="primary"  @click="cost_getData(id)">{{$t('button.ser')}}</Button>
            <Button type="warning"   @click="clear(2)">清空</Button>
          </Row>
          <Row class="table-form">
            <Col span="19">
          ...
          <Row style="display: flex">          <Page :total="cost_total" size="small" show-elevator show-sizer placement="top" :page-size-opts = "[10, 20, 50, 100]" ></Page>
      <span style="margin-left: 10px;margin-top: 2px"> {{$t('reminder.view')}} {{((page - 1) * rows + 1)}} - {{page*rows > total ? total : page*rows}}  {{$t('reminder.of')}}  <span>{{total}}</span></span><Button size="small" shape="circle" icon="md-refresh" style="margin-left: 20px;display: inline-block;" @click="getData(1)"></Button></Row>
        </TabPane>
      </Tabs>-->
      <!-- 选项卡 结束-->
