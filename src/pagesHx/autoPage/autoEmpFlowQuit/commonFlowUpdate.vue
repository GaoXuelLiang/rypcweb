                ...
                <div v-if="stepOrApproval === '0'"
                    ...
                    <div class="footer"
                        ...
                        <div class="footerChilden">
                            ...
                            <Button type="primary"
                                    @click="save"
                                    v-show="stepAuthLimits=='02update'||stepAuthLimits=='03submit'"
                                    :disabled="thisDis"
                                    :loading="loading1"><span v-show="!loading1">{{$t('button.sav')}}</span></Button>
                            <Button type="primary"
                                    :loading="loading1"
                                    @click="sendSign"
                                    v-show="qysBtn"><span v-show="!loading1">发送电子签</span></Button>
                            <Button type="success"
                                    style="margin-left: 5px;"
                                    @click="isSubmit"
                                    :disabled="thisDis"
                                    :loading="loading1"
                                    v-show="thisPkValue !== '0'&&stepAuthLimits=='03submit'"><span v-show="!loading1">提交</span></Button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    methods: {
        ...
        getFormDataSubmit (data, res) {
            ...
            for (let i = 0; i < columns.length; i++) {
                ...
                if (columns[i].clmLayout == 13 || columns[i].clmLayout == 12 || columns[i].clmLayout == 11 || columns[i].clmLayout == 1 || columns[i].clmLayout == 3 || columns[i].clmLayout == 90 || columns[i].clmLayout == 92) {
        ...
        getData () {
            ...
            getDataLevelUserLoginData({
                ...
                APlogType: 'getPage',
                APffk: `glc${this.funId}|${this.flowId}|${this.userId}`,
        ...
        isSubmit () {
            ...
            if (termroleType === "3user" && this.stepCode === "flow_termination_1100") {
                ...
                for(let ref in t.$refs) {
                    if( ref.indexOf('subtable') > -1) {
                        if(t.$refs[ref][0]) {
                            if(t.$refs[ref][0].tbName === "EmpQuithfile" || t.$refs[ref][0].tbName === "EmpQuitifile") {
                                if(t.$refs[ref][0].data.length === 0) {
                                    t.$Modal.warning({
                                        title: '警告',
                                        content: `请完善${t.$refs[ref][0].titleName}`
                                    })
                                    return;
                                }
                            }
