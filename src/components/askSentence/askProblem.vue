<template>
  <div class="cover">
    <div class="box">
      ...
      <Row style="font-size: 14px;margin-bottom: 10px;">已有相似问法(<span style="font-weight: bold;">{{data.length}}</span>)</Row>
      <Row style="font-size: 14px;margin-bottom: 10px;" v-show="typee === '1'">
        <Input placeholder="请输入相似问法添加" style="width: 300px" v-model.trim="content"/>
  ...
  export default {
    ...
    methods: {
      getData(id, name, type) {
        ...
        t.askId = id || t.askId
        t.typee = t.typee || type
        t.data = []
        getDataLevelUserLogin({
          _mt:  this.$global.mt+'CmutKnowledgeQuest.getByKldid',
          logType: '相似问法',
          id: t.askId,
        }).then((res) => {
          if (isSuccess(res, t)) {
      ...
      addAsk() {
        ...
        if (!t.content) {
          ...
          getDataLevelUserLoginNew({
            _mt:  this.$global.mt+'CmutKnowledgeQuest.addOrUpd',
            logType: '相似问法新增',
            cmutKldqstKldid: t.askId,
            cmutKldqstQuestion: t.content,
          }).then((res) => {
            if (isSuccess(res, t)) {
              t.getData()
            }
      ...
      deleteAsk() {
        ...
        if (t.tableselected.length === 0) {
          ...
          t.$Modal.confirm({
            ...
            onOk: () => {
              getDataLevelUserLogin({
                _mt:  this.$global.mt+'CmutKnowledgeQuest.delByIds',
                funId: '1',
                companyId: '1',
