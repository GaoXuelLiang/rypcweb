...
//                console.log(this.columns, 'pages')
...
//              console.log(this.columns, 'columns')
    ...
    methods: {
        ...
        getData (page, params) {
            ...
            if (params) {
              data.searchData = JSON.stringify(params) // 用做高级查询 例：员工奖惩管理
            }
            for (let k in this.filterParams) {
                data[k] = this.filterParams[k]
            }
        ...
        modifystatus (state, tipName) {
            ...
            for (let v in data) {
              if (data[v] === '' || data[v] === undefined || data[v] === 'undefined') {
                delete data[v]
              }
            }
            t.$Modal.confirm({
                    ...
                    getDataLevelUserLogin(data)
                        .then(res => {
                            if (isSuccess(res, t)) {
                                t.getData();
                                t.tableselected = [];
                                this.$Message.success(this.$t("reminder.opesuccess"));
                            }
                        })
                        .catch(() => {
                            t.tableselected = [];
