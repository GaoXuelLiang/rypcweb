    ...
    <div class="cover">
        <div class="box">
            ...
            <Row type="flex"
                ...
                <i-col span="24"
                       class="col">
                    <span style="font-weight: bold;margin-right:10px">请选择子表类型 ：</span>
                    <i-col span="5"
                       class="col">
                    <Select v-model="childType"
                                :clearable='true'
                                 @on-change='changeChildType'
                                placeholder="请选择子表类型">
...
export default {
    ...
    methods: {
        changeChildType(value){
            const t = this 
            if(value == '' || value== undefined){
                 this.$Message.warning({
                    content: '请先选择需要导入的子表类型'
                });
                return
            }
        },
        ...
        getSelect () {
            ...
            getDataLevelUserLoginData({
                ...
                APtypeCode: 'empFormtype',
            }).then((res) => {
                if (isSuccess(res, t)) {
                    t.childtypeList = res.data.content[0].value[0].paramList
                }
            }).catch(() => {
                this.$Modal.error({
        ...
        handleReset () {
            ...
            this.$emit('closePrimarydata')
        },
        closeImportFail () {
            this.openImportFail = false
        },
        handleUpload (file) {
            if(this.childType==''||this.childType==undefined){
                this.$Message.warning({
                    content: '请先选择需要导入的子表类型'
                });
                return
