    ...
    <div class="backbox">
      <div class="title">
        <div class="title-text">
          <Icon type="ios-bookmark" size="16"></Icon>&nbsp;{{$t('lang_payroll.paySalSetItem.salItemColumn')}}
        </div>
      ...
      <Row class="table-form">
        <Input placeholder="请输入项目编码" style="width: 200px" v-model="paysiCode"/>
  ...
  export default{
    ...
    methods: {
      ...
      dbCkick(row) {
        const t = this
        this.close()
        this.$emit('changeinput', row[t.searchCloumns[0].key], row.id, row)
        this.$emit('closeUp')
      },
      sizeChange(size) {
        const t = this
