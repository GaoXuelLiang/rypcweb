    ...
    <div class="backbox">
      <div class="title">
        <div class="title-text">
          <Icon type="ios-bookmark" size="16"></Icon>选择Hrbp对接人
        </div>
      ...
      <Row class="table-form">
        <Input placeholder="请输入对接人" class="searchInput" style="width: 200px" v-model="hrbpCname"/>
      ...
      <row class="table-form" ref="table-form">
        <Table height="320" size="small" stripe :no-data-text="noDataImg" ref="selection" :columns="searchHrbpClo" @on-sort-change="sortable" :data="data" @on-row-dblclick="dbClick"></Table>
      </row>
  ...
  export default{
    ...
    methods: {
      ...
      dbClick(row) {
      	const t = this
        this.close()
      	t.$emit('inputHrbp', row.empName, row.id)
      	t.$emit('closeHrbp')
      },
    	close() {
    		const t = this
