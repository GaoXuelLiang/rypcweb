    ...
    <div class="backbox">
      <div class="title">
        <div class="title-text">
          <Icon type="ios-bookmark" size="16"></Icon>&nbsp;邮件模板
        </div>
      ...
      <Row class="table-form">
        <Input placeholder="请输入邮件名称..." style="width: 200px" v-model="sysmailName"/>
      ...
      <row class="table-form" ref="table-form">
        <Table height="320" size="small" border ref="selection" :columns="searchMail" @on-sort-change="sortable" :data="data" @on-row-dblclick="dbCkick"></Table>
      </row>
    ...
    methods: {
        ...
        if (page) {
          params.page = page
			...
			data.sysmailName = t.sysmailName
    	...
    	dbCkick(row) {
    		const t = this
        this.close()
	        t.$emit('inputMail', row[t.searchMail[0].key], row.id)
	        t.$emit('closeMail')
    	},
    	sortable(column) {
