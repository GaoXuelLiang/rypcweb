    ...
    <div class="backbox">
      <div class="title">
        <div class="title-text">
          <Icon type="ios-bookmark" size="16"></Icon>&nbsp;调查变量
        </div>
      ...
      <Row class="table-form">
        <Input placeholder="请输入变量名称..." style="width: 200px" v-model="platSvaCname"/>
      ...
      <row class="table-form" ref="table-form">
        <Table height="320" size="small" border ref="selection" :columns="searchSurVar" @on-sort-change="sortable" :data="data" @on-row-dblclick="dbCkick"></Table>
      </row>
    ...
    methods: {
        ...
        if (page) {
          params.page = page
			...
			data.platSvaCname = t.platSvaCname
    	...
    	dbCkick(row) {
    		const t = this
        this.close()
	        t.$emit('inputSurveyVar', row[t.searchSurVar[0].key], row.id)
	        t.$emit('closeSurVar')
    	},
    	sortable(column) {
