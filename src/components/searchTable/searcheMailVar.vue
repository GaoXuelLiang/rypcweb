    ...
    <div class="backbox">
      <div class="title">
        <div class="title-text">
          <Icon type="ios-bookmark" size="16"></Icon>&nbsp;邮件变量
        </div>
      ...
      <Row class="table-form">
        <Input placeholder="请输入邮件变量名称..." style="width: 200px" v-model="sysmlvarName"/>
      ...
      <row class="table-form" ref="table-form">
        <Table height="320" size="small" border ref="selection" :columns="searchMailVar" @on-sort-change="sortable" :data="data" @on-row-dblclick="dbCkick"></Table>
      </row>
...
<script>
  ...
  export default{
    ...
    methods: {
        ...
        data.sysmlvarName = t.sysmlvarName
    	...
    	dbCkick(row) {
    		const t = this
        this.close()
	        t.$emit('inputMailVar', row[t.searchMailVar[0].key], row.id)
	        t.$emit('closeMailVar')
    	},
    	sortable(column) {
