    ...
    <div class="backbox">
      <div class="title">
        <div class="title-text">
          <Icon type="ios-bookmark" size="16"></Icon>&nbsp;短信模板
        </div>
      ...
      <Row class="table-form">
        <Input placeholder="请输入模板编号..." style="width: 200px" v-model="syssmsNo"/>
      ...
      <row class="table-form" ref="table-form">
        <Table height="320" size="small" border ref="selection" :columns="searchSms" @on-sort-change="sortable" :data="data" @on-row-dblclick="dbCkick"></Table>
      </row>
  ...
  export default{
    ...
    methods: {
    	...
    	dbCkick(row) {
    		const t = this
        this.close()
	        t.$emit('inputSms', row[t.searchSms[1].key], row.id)
	        t.$emit('closeSms')
    	},
    	sortable(column) {
