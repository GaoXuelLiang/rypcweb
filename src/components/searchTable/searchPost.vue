            ...
            <div class="title">
                <div class="title-text">
                    <Icon type="ios-bookmark"
                          size="16"></Icon>&nbsp;{{$t('lang_employee.searchColumn.post')}}
                </div>
                <Button type="text"
                        @click="close">
                    <Icon type="md-close"
            ...
            <Row class="table-form">
                ...
                <span class="btn">
                    <Button type="info"
                            ...
                            style="width:56px"
                            :loading="isSpin"
                            @click="getData(params,id, 1)">
                        <span v-if="!isSpin">{{$t('button.ser')}}</span>
                    </Button>
                </span>
                <span class="btn"><Button type="warning"
                            class="btnStyle"
...
export default {
    ...
    methods: {
        getData (params, id, page) {
            ...
            if (id) {
                t.params.unptDeptid = id
            }
            const data = deepCopy(t.params)
            data.unptPostidDis = t.name
        ...
        dbCkick (row) {
            const t = this
            this.close()
            t.$emit('inputPost', row.unptPostidDis, row.unptPostid, row)
            t.$emit('closePost')
        },
        sortable (column) {
