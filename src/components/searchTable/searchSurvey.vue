    ...
    <div class="backbox">
      <div class="title">
        <div class="title-text">
          <Icon type="ios-bookmark" size="16"></Icon>&nbsp;调查定义
        </div>
      ...
      <Row class="table-form">
        <Input placeholder="请输入调查编号..." style="width: 200px" v-model="platQusDesccn"/>
      ...
      <row class="table-form" ref="table-form">
        <Table height="320" size="small" border ref="selection" :columns="searchSur" @on-sort-change="sortable" :data="data" @on-row-dblclick="dbCkick"></Table>
      </row>
  ...
  export default{
    ...
    methods: {
        ...
        if (page) {
          ...
          data.platSuvCode = t.platQusDesccn
    	...
    	dbCkick(row) {
    		const t = this
        this.close()
	        t.$emit('inputSurvey', row[t.searchSur[1].key], row.id)
	        t.$emit('closeSur')
    	},
    	sortable(column) {
