    ...
    <div class="backbox">
      <div class="title">
        <div class="title-text">
          <Icon type="ios-bookmark" size="16"></Icon>&nbsp;微信变量
        </div>
      ...
      <Row class="table-form">
        <Input placeholder="请输入微信变量名称..." style="width: 200px" v-model="syswxvarName"/>
      ...
      <row class="table-form" ref="table-form">
        <Table height="320" size="small" border ref="selection" :columns="searchWxVar" @on-sort-change="sortable" :data="data" @on-row-dblclick="dbCkick"></Table>
      </row>
    ...
    methods: {
        ...
        if (page) {
          params.page = page
			...
			data.syswxvarName = t.syswxvarName
    	...
    	dbCkick(row) {
    		const t = this
        this.close()
	        t.$emit('inputWxVar', row[t.searchWxVar[0].key], row.id)
	        t.$emit('closeWxVar')
    	},
    	sortable(column) {
