                ...
                <Row>
                    ...
                    <Row class="btnHeight">
                        <span v-for="(item, index) in searchInp"
                            ...
                            <DatePicker v-if="item.clmLayout == 76"
                                        v-model="item.name"
                                        type="daterange"
                                        format="yyyy-MM-dd"
                                        :placeholder="'请选择' + item.clmDname"
                                        @on-change="daterangeChange(item)"
                                        :editable="false"
                                        class="table_search_int pickData"></DatePicker>
                        ...
                        <Button class="btns"
                            ...
                            <span v-show="item.code==btnCode?!isSpinTable:true">
                                {{ item.name }}
                            </span>
                        </Button>
...
export default {
    ...
    mounted () {
        console.log("autoMain3333333333333333333333333333333333333333333333")
        this.initData()
    },
    methods: {
        //初始化数据
        ...
        //列表点击
        tableClick (id, colimnKey, params) {
            const t = this;
            this.$emit("specialButton", colimnKey, t, params)
        },
        //操作按钮
        ...
        exportEvent () {
            ...
            if (this.isTree) {
                treeType = this.judTreeType == '05unittree' ? 'unitPid' : 'cityPid'
                dataSearch[treeType] = this.treeid
            }
            ...
            getDataLevelUserLoginData({
                ...
                if (isSuccess(res, t)) {
                    ...
                    if (res.data.content[0].funIsrpt == '1') {
                        t.tbName = "Ansrpt" + res.data.content[0].tbName
                    } else {
                    ...
                    for (let i in aa) {
                        ...
                        if (aa[i].key === 'statusDis' && t.tbName === "AttMoncheckHandle") {
                            aa[i].fixed = 'right'
                        }
                        aa[i]['render'] = (h, params) => {
                            ...
                            if (!aa[i].islbclk || aa[i].islbclk === '0') {
                                return h('Tooltip', {
                                    props: {
                                        trigger: 'hover',
                                        content: tt,
                                ...
                                return h('div', {
                                    ...
                                    h('a', {
                                        ...
                                        on: {
                                            click: () => {
                                                this.tableClick(params.row.id, aa[i].key, params.row)
                                            }
                                        }
                                    }, tt),
                                ], '')
                ...
                arr.push(
                    h('Button', {
                        ...
                        style: {
                            ...
                            display: (item.btn_id != 'button_y_entry') || (item.btn_id == 'button_y_entry' && params.row.maemReject == '1' && params.row.status == "05zbemp") || (item.btn_id == 'button_y_entry' && params.row.status == "02jsoffer") ? "inline-block" : "none"
                        },
                        on: {
                            click: () => {
        ...
        getData (ctree) {
            ...
            if (t.homePageFiltering !== '' && t.homePageFiltering !== undefined) {
                if (t.homePageFiltering ==='selfService') {
                    data.empId = t.empId
                }else if(t.homePageFiltering==='selfServiceMyteam'){
                    data.teamType ="2"
                    data.empZpid =  t.empId
                } else {
                    ...
                    homePage.forEach((val, index) => {
                        if (val !== '') {
                            ...
                            if (!data.hasOwnProperty(sp[0])) {
                                if (sp[0] === 'selfService') {
                                    data.empId = t.empId
                                }else if(sp[0] === 'selfServiceMyteam'){
                                        data.teamType ="2"
                                        data.empZpid =  t.empId
                                } else {
                                    data[sp[0]] = sp[1]
            ...
            if (t.judTreeType == '09citytree') {
                ...
                if (pubsource.param_mt === 'znjs' && ctree === "1") {
                    data.APcitySelf = '1'
                }
            } else if (t.judTreeType == '05unittree') {
                if(t.tbName ==='EmpInfo'){
                    data.empUnitid = t.treeid
                }else{
                    data.unitPid = t.treeid;
                    if (pubsource.param_mt === 'znjs') {
                        data.qcIncludeSelf = '1'
                    }
                }
            }
            if (t.showBtnStatus) {
        ...
        changeMsg (mt, reminder, type, code) {
            ...
            if (t.tableselected.length === 0) {
                ...
                // } else if (t.tableselected.length > 1 && this.$global.mt === 'znjs') {
                //     t.$Modal.warning({
                //         title: this.$t('reminder.remind'),
        ...
        daterangeChange (item) {
            if (item.name) {
                ...
                let ids = []
                Array.forEach((item) => {
                    ids.push(new Date(item).format('yyyy-MM-dd'))
                })
                item.name = ids
            }
        },
